...
my $trace_id = time();
$r = withdraw(loginid => $loginid, trace_id => $trace_id, amount => 1);
is($r->code,    201,       'correct status code');
is($r->message, 'Created', 'Correct message');
...
$r = request(
    'POST', '/transaction/payment/doughflow/withdrawal_reversal',
    {
        amount            => 1,
...
like $r->decoded_content, qr/no corresponding original withdrawal could be found/,
                                                    'reject withdrawal reversal using bad trace_id nicely';
...
like $r->decoded_content,
    qr/Bonuses and fees are not allowed for withdrawal reversals/,
    '.. no bonuses or fees allowed here';
