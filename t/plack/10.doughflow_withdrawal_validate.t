...
$r = withdraw(loginid => $loginid, is_validate => 1);
...
$r = withdraw(loginid => $loginid, amount => 'Zzz', is_validate => 1);
is($r->code,    400,           'Correct failure status code');
like($r->content, qr[(Attribute \(amount\) does not pass the type constraint|Invalid money amount)],
        'Correct error message on response body');

my $balance_now = balance($loginid);
$r = withdraw(loginid => $loginid, amount => $balance_now + 1, is_validate => 1);
is $r->code, 403, 'withdraw rejected';
like $r->decoded_content, qr/exceeds client balance/, "$balance_now plus 1 exceeds balance now";

# < 0.01 or > 100000
$r = withdraw(loginid => $loginid, amount => 0.009, is_validate => 1);
...
$r = withdraw(loginid => $loginid, amount => 100001, is_validate => 1);
