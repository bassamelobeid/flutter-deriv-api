...
subtest $method => sub {
    ...
    cmp_bag($c->tcall($method, {token => '12345'}), [qw(USD EUR GBP AUD BTC LTC)], 'invalid token will get all currencies');
    cmp_bag(
        $c->tcall(
            $method,
            {
    ...
    cmp_bag($c->tcall($method, {}), [qw(USD EUR GBP AUD BTC LTC)], "will return legal currencies if no token");
};
