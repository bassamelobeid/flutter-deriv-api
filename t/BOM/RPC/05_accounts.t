...
subtest $method => sub {
    is_deeply($c->tcall($method, {token => '12345'}), [qw(USD EUR GBP AUD BTC ETH)], 'invalid token will get all currencies');
    is_deeply(
        $c->tcall(
            $method,
            {
        ...
        [qw(USD EUR GBP AUD BTC ETH)],
        'undefined token will get all currencies'
    );

    is_deeply($c->tcall($method, {token => $token_21}), ['USD'], "will return client's currency");
    is_deeply($c->tcall($method, {}), [qw(USD EUR GBP AUD BTC ETH)], "will return legal currencies if no token");
};
