...
subtest general => sub {
    ...
    my $res = $o->create_order({client => $client_fiat, source_type=>'official'})->get();
    cmp_deeply (
        $res->{error}{error},
        { 
    ...
    $res = $o->create_order({client => $client_fiat, source_type=>'official'})->get();
    cmp_deeply (
        $res->{error}{error},
        { 
            code => 'OrderCreationError',
...
subtest banxa => sub {
    ...
    my $config = { api_url => 'dummy', api_key => '12345', api_secret => 'topsecret' };
    ...
    $res = $o->create_order({client => $client_fiat, source_type=>'unofficial'})->get();
    ...
    $res = $o->create_order({client => $client_crypto, source_type=>'official', referrer=>$referrer})->get;
    ...
    $res = $o->create_order({client => $client_crypto, source_type=>'official', args => {referrer=>$referrer}})->get();
...
subtest wyre => sub {
    ...
    my $config = { api_url => 'dummy', api_account_id => '12345', api_secret => 'topsecret' };
    ...
    my $res = $o->create_order({client => $client_crypto, source_type=>'official'})->get();
    cmp_deeply(
        $res->{error}{error},
        { 
            code => 'ConnectionError',
    ...
    $res = $o->create_order({client => $client_crypto, source_type=>'official'})->get();
    cmp_deeply(
        $res->{error}{error},
        { 
            code => 'OrderCreationError',
    ...
    my $wyre_resp = { reservation => '8dj3mikv9uG', url => 'www.wyre.com/checkout' };
