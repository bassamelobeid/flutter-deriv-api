...
my ($token) = BOM::Database::Model::OAuth->new->store_access_token_only( 1,
    $client->loginid );
...
my $c =
  BOM::Test::RPC::Client->new( ua => Test::Mojo->new('BOM::RPC')->app->ua );
request( BOM::Platform::Context::Request->new( params => {} ) );

subtest 'validate_symbol' => sub {
    is( BOM::RPC::v3::Contract::validate_symbol('R_50'),
        undef, "return undef if symbol is valid" );
    cmp_deeply(
...
subtest 'validate_license' => sub {
    is( BOM::RPC::v3::Contract::validate_license('R_50'),
        undef, "return undef if symbol is realtime" );
...
subtest 'validate_is_open' => sub {
    ...
    is( BOM::RPC::v3::Contract::validate_is_open('R_50'),
        undef, "Random is always open" );
...
subtest 'validate_underlying' => sub {
    ...
    cmp_deeply(
        BOM::RPC::v3::Contract::validate_underlying('R_50'),
        { status => 1 },
        'status 1 if everything ok'
    );
};
...
subtest 'prepare_ask' => sub {
    ...
    cmp_deeply( BOM::RPC::v3::Contract::prepare_ask($params),
        $expected, 'prepare_ask result ok' );
    $params = {
        %$params,
    ...
    cmp_deeply( BOM::RPC::v3::Contract::prepare_ask($params), $expected,
'will set barrier default value and delete barrier2 if contract type is not like SPREAD and ASIAN'
    );
    ...
    for my $t (qw(SPREAD ASIAN)) {
        ...
        cmp_deeply( BOM::RPC::v3::Contract::prepare_ask($params),
            $expected,
            'will not set barrier if contract type is like SPREAD and ASIAN ' );
...
subtest 'get_ask' => sub {
    ...
    my $result =
      BOM::RPC::v3::Contract::_get_ask(
        BOM::RPC::v3::Contract::prepare_ask($params) );
...
'Win payout if Volatility 50 Index is strictly higher than entry spot at 1 minute after contract start time.',
        ...
        'contract_parameters' => {
            ...
            'staking_limits'             => {
                'message_to_client' =>
                  'Minimum stake of 0.35 and maximum payout of 50,000.00',
                'min'                     => '0.35',
                'max'                     => 50000,
...
subtest 'send_ask' => sub {
    ...
    my $expected_keys = [
        sort { $a cmp $b } (
            qw(longcode spot display_value ask_price spot_time date_start rpc_time payout theo_probability contract_parameters)
        )
    ];
    cmp_deeply(
        [ sort keys %$result ],
        $expected_keys,
        'result keys is correct'
    );
    is(
        $result->{longcode},
'Win payout if Volatility 50 Index is strictly higher than entry spot at 1 minute after contract start time.',
    ...
    );
    {
        cmp_deeply(
            [
                warnings {
                    $c->call_ok( 'send_ask', { args => { symbol => 'R_50' } } )
                      ->has_error->error_code_is('ContractCreationFailure')
                      ->error_message_is('Cannot create contract');
                }
            ],
            bag(
                re('Use of uninitialized value in pattern match'),
                re('_get_ask produce_contract failed')
            ),
            'missing bet_type when checking contract_type'
        );

        my $mock_contract = Test::MockModule->new('BOM::RPC::v3::Contract');
        $mock_contract->mock( '_get_ask',
            sub { die "mock _get_ask dying on purpose" } );
        cmp_deeply(
            [
                warnings {
                    $c->call_ok( 'send_ask', { args => { symbol => 'R_50' } } )
                      ->has_error->error_code_is('pricing error')
...
subtest 'send_multiple_ask' => sub {
    ...
    use Data::Dumper;
    my $result =
      $c->call_ok( 'send_multiple_ask', $params )->has_no_error->result;
    ...
    my $inner_expected_keys = [
        sort (
            qw(longcode spot display_value spot_time ask_price date_start contract_parameters payout barrier)
        ) ];
    for my $proposal ( @{ $result->{proposals} } ) {
        cmp_deeply( [ sort keys %$proposal ],
            $inner_expected_keys, 'result keys is correct in proposals' );
    }
    ...
    is( scalar( @{ $result->{proposals} } ), 2, "There are 2 proposals" );
    cmp_deeply(
        [ sort keys %{ $result->{proposals}[0] } ],
        [ sort qw(error) ],
        'the first proposal has error'
    );
    my $expected_error_keys =
      [ sort qw(message_to_client details continue_price_stream code) ];
    cmp_deeply( [ sort keys %{ $result->{proposals}[0]{error} } ],
        $expected_error_keys, "error keys ok" );
    my $expected_details_keys = [ sort qw(display_value payout barrier) ];
    cmp_deeply( [ sort keys %{ $result->{proposals}[0]{error}{details} } ],
        $expected_details_keys, "details keys ok" );
    my $proposal = $result->{proposals}[1];
    cmp_deeply( [ sort keys %$proposal ],
        $inner_expected_keys, 'the second proposal still ok' );
};
...
subtest 'get_bid' => sub {
    ...
    $c->call_ok( 'get_bid', $params )
      ->has_error->error_code_is('GetProposalFailure')
      ->error_message_is(
'There was a market data disruption during the contract period. For real-money accounts we will attempt to correct this and settle the contract properly, otherwise the contract will be cancelled and refunded. Virtual-money contracts will be cancelled and refunded.'
    ...
    $result = $c->call_ok( 'get_bid', $params )
      ->has_no_system_error->has_no_error->result;
...
subtest 'get_bid_skip_barrier_validation' => sub {
    ...
    $result = $c->call_ok( 'get_bid', $params )
      ->has_no_system_error->has_no_error->result;
    like $result->{validation_error}, qr/^Barrier must be at least/,
      "Barrier error expected";

    $params->{validation_params} = { skip_barrier_validation => 1 };
    $result = $c->call_ok( 'get_bid', $params )
      ->has_no_system_error->has_no_error->result;
    ok( !exists $result->{validation_error}, "No barrier validation error" )
      or diag "validatione error: "
      . ( $result->{validation_error} // '<undef>' );
...
subtest $method => sub {
    ...
    $c->call_ok( $method, $params )
      ->has_error->error_message_is( 'The token is invalid.', 'invalid token' );
    $client->set_status( 'disabled', 1, 'test' );
    $client->save;
    $params->{token} = $token;
    $c->call_ok( $method, $params )
      ->has_error->error_message_is( 'This account is unavailable.',
        'invalid token' );
    ...
    cmp_deeply(
        [
            warnings {
                $c->call_ok( $method, $params )
                  ->has_error->error_message_is( 'Cannot create contract',
...
"Win payout if Volatility 50 Index is strictly higher than entry spot at 50 seconds after contract start time.",
    ...
    create_ticks(
        [ 0.9935, $now->epoch - 899, 'frxAUDCAD' ],
        [ 0.9938, $now->epoch - 501, 'frxAUDCAD' ],
        [ 0.9939, $now->epoch - 499, 'frxAUDCAD' ]
    );
    ...
    my $expected_result = {
        ...
        'longcode' =>
'Win payout if AUD/CAD is strictly higher than entry spot at 6 minutes 40 seconds after contract start time.',
    ...
    foreach my $key ( keys %$expected_result ) {
        cmp_ok $res->{$key}, 'eq', $expected_result->{$key},
          "$key are matching ";
    }

    create_ticks(
        [ 0.9936, $now->epoch - 499, 'frxAUDCAD' ],
        [ 0.9938, $now->epoch - 100, 'frxAUDCAD' ],
        [ 0.9934, $now->epoch - 99,  'frxAUDCAD' ]
    );
    ...
    $expected_result = {
        ...
        'longcode' =>
'Win payout if AUD/CAD is strictly higher than entry spot at 6 minutes 42 seconds after contract start time.',
...
subtest 'get_bid_affected_by_corporate_action' => sub {
    ...
    Quant::Framework::CorporateAction::create( $storage_accessor, 'USAAPL',
        $opening )->update( $action, $opening )->save;
  ...
  SKIP: {
        my $expected_result = {
            ...
            'validation_error' =>
              'This contract is affected by corporate action.',
            'currency'         => 'USD',
            'underlying'       => 'USAAPL',
            'entry_tick'       => '111.00',
            ...
            'bid_price'           => '0.00',
            'longcode' =>
'Win payout if Apple is strictly lower than entry spot at close on 2005-09-24.',
        ...
        foreach my $key ( keys %$expected_result ) {
            cmp_ok $result->{$key}, 'eq', $expected_result->{$key},
              "$key are matching ";
        }
    }
};
...
subtest 'app_markup_percentage' => sub {
    ...
    $result =
      BOM::RPC::v3::Contract::_get_ask(
        BOM::RPC::v3::Contract::prepare_ask($params), 1 );
    is $result->{ask_price} - $val, 1 / 100 * 100,
      "as app markup is added so client has to 1% of payout";
    ...
    $result =
      BOM::RPC::v3::Contract::_get_ask(
        BOM::RPC::v3::Contract::prepare_ask($params), 2 );
    cmp_ok $val - $result->{payout}, ">", 2 / 100 * $val,
"as app markup is added so client will get less payout as compared when there is no markup";
    ...
    $result = $c->call_ok( 'get_bid', $params )
      ->has_no_system_error->has_no_error->result;
    is $contract->payout, $result->{payout},
      "contract and get bid payout should be same when app_markup is included";
    ...
    cmp_ok $contract->payout, ">", $result->{payout},
"payout in case of stake contracts would be higher as compared to app_markup stake contracts";
