...
$loginids{ez_demo} = $c->call_ok('trading_platform_new_account', $params)->result->{login};
...
$loginids{ez_real} = $c->call_ok('trading_platform_new_account', $params)->result->{login};
...
subtest 'account list' => sub {
    ...
    cmp_deeply \@logins, bag(@loginids{qw(cr_usd cr_btc mt5_gaming mt5_financial dx_real ez_real ct_real)}), 'CR gets all real accounts';

    # link all trading accounts to DF wallet and create a linked CR standard account
    $user->link_wallet_to_trading_account({wallet_id => $loginids{vrw},    client_id => $_}) for @loginids{qw(mt5_demo dx_demo ez_demo ct_demo)};
    $user->link_wallet_to_trading_account({wallet_id => $loginids{crw_df}, client_id => $_})
        for @loginids{qw(mt5_gaming mt5_financial dx_real ez_real ct_real)};
    ...
    $res             = $c->call_ok('transfer_between_accounts', $params)->result;
    @logins          = map { $_->{loginid} } $res->{accounts}->@*;
    cmp_deeply \@logins, bag(@loginids{qw(vrw vrtc_trading mt5_demo dx_demo ez_demo ct_demo)}), 'expected accounts for VRW';
    ...
    $res             = $c->call_ok('transfer_between_accounts', $params)->result;
    @logins          = map { $_->{loginid} } $res->{accounts}->@*;
    cmp_deeply \@logins, bag(@loginids{qw(crw_df crw_btc cr_standard crw_p2p crw_pa mt5_gaming mt5_financial ez_real dx_real ct_real)}),
...
subtest 'Virtual transfers' => sub {
    ...
    $expected{a}->{balance} = num(990);
    $expected{b} = {
        loginid          => $loginids{ez_demo},
        balance          => num(10010),           # derivEZ demo accounts get created with 10k initial balance for now
        account_type     => 'derivez',
        account_category => 'trading',
        currency         => 'USD',
        transfers        => 'all',
        demo_account     => bool(1),
        derivez_group    => ignore(),
    };

    $params->{args}{account_from} = $loginids{vrw};
    $params->{args}{account_to}   = $loginids{ez_demo};
    $res                          = $c->call_ok('transfer_between_accounts', $params)->has_no_error('VRW to DerivEZ ok')->result;
    cmp_deeply $res->{accounts}, bag(values %expected), 'VRW to DerivEZ expected response';

    $params->{args}{account_from} = $loginids{ez_demo};
    $params->{args}{account_to}   = $loginids{vrw};
    $res                          = $c->call_ok('transfer_between_accounts', $params)->has_no_error('DerivEZ to VRW ok')->result;
    $expected{a}->{balance}       = num(1000);
    $expected{b}->{balance}       = num(10000);
    cmp_deeply $res->{accounts}, bag(values %expected), 'DerivEZ to VRW expected response';
    ...
    cmp_ok $user->daily_transfer_count(
        ...
        '==', 10, 'virtual transfer count incremented for user';
    cmp_ok $user->daily_transfer_amount(
        type       => 'virtual',
        identifier => $user->id
        ),
        '==', 100, 'virtual transfer amount incremented for user';

    # errors
    $app_config->payments->transfer_between_accounts->daily_cumulative_limit->enable(0);
    $app_config->payments->transfer_between_accounts->limits->virtual(10);

    $params->{token} = $tokens{vrw};
    $params->{args}{account_from} = $loginids{vrw};

    for my $l (qw(vrtc_trading mt5_demo dx_demo ez_demo ct_demo)) {
        ...
        $c->call_ok('transfer_between_accounts', $params)->error_code_is('MaximumTransfers', "vrw to $l error code for exceed transfer count")
            ->error_message_like(qr/up to 10 transfers/, "vrw to $l error message for exceed transfer count")->result;
    }

    $app_config->payments->transfer_between_accounts->daily_cumulative_limit->enable(1);
    $app_config->payments->transfer_between_accounts->daily_cumulative_limit->virtual(100);
    ...
    for my $l (qw(vrtc_trading mt5_demo dx_demo ez_demo ct_demo)) {
        ...
        $c->call_ok('transfer_between_accounts', $params)->error_code_is('MaximumAmountTransfers', "vrw to $l error code for exceed transfer amount")
            ->error_message_like(qr/100.00 USD per day/, "vrw to $l error message for exceed transfer amount");
    }
    ...
    @tests = (
        ...
        [ez_demo       => 'TransferBlockedWalletNotLinked'],
        ...
        [ez_real       => 'RealToVirtualNotAllowed'],
    ...
    @tests = (
        ...
        [ez_demo       => 'TransferBlockedTradingAccounts'],
        [dx_demo       => 'DXInvalidAccount'],
        [ct_demo       => 'CTraderInvalidAccount'],
        [crw_df        => 'RealToVirtualNotAllowed'],
        [crw_btc       => 'RealToVirtualNotAllowed'],
...
subtest 'real transfers' => sub {
    ...
    $params->{args} = {
        account_from => $loginids{crw_df},
        account_to   => $loginids{ez_real},
        amount       => 10,
        currency     => 'USD',
    };

    $res = $c->call_ok('transfer_between_accounts', $params)->has_no_error('CRW to Deriv EZ ok')->result;
    cmp_deeply $res->{accounts}, bag(values %expected), 'CRW to Deriv EZ expected response';
    cmp_ok $user->daily_transfer_count(
        type       => 'derivez',
        identifier => $loginids{crw_df}
        ),
        '==', 1, 'derivez transfer count incremented for wallet';
    cmp_ok $user->daily_transfer_amount(
        type       => 'derivez',
        identifier => $loginids{crw_df}
        ),
        '==', 10, 'derivez transfer amount incremented for wallet';

    $params->{args}{account_from} = $loginids{ez_real};
    $params->{args}{account_to}   = $loginids{crw_df};
    $expected{a}->{balance}       = num(980);
    $expected{b}->{balance}       = num(0);

    $res = $c->call_ok('transfer_between_accounts', $params)->has_no_error('MT5 to Deriv EZ ok')->result;
    cmp_deeply $res->{accounts}, bag(values %expected), 'MT5 to Deriv EZ expected response';
    cmp_ok $user->daily_transfer_count(
        type       => 'derivez',
        identifier => $loginids{crw_df}
        ),
        '==', 2, 'derivez transfer count incremented for wallet';
    cmp_ok $user->daily_transfer_amount(
        type       => 'derivez',
        identifier => $loginids{crw_df}
        ),
        '==', 20, 'derivez transfer amount incremented for wallet';
    ...
    @tests = (
        ...
        [dx_real       => 'DXInvalidAccount'],
        [ct_real       => 'CTraderInvalidAccount'],
        [ez_real       => 'TransferBlockedWalletNotLinked'],
    );
    run_tests('crw_btc');
    ...
    @tests = (
        ...
        [dx_real       => 'DXInvalidAccount'],
        [ct_real       => 'CTraderInvalidAccount'],
        [ez_real       => 'TransferBlockedTradingAccounts'],
    );
    ...
    for my $l (qw(cr_standard crw_btc crw_p2p mt5_gaming dx_real ez_real ct_real)) {
        $app_config->payments->transfer_between_accounts->daily_cumulative_limit->enable(0);
