...
subtest 'Account Opening (new) Verification' => sub {
    ...
    is $verification->{account_opening_new}->()->{message}, get_verification_message('account_opening_new', 'signup'), 'Account opening with verification URI';
};

subtest 'Payment Withdraw Verification' => sub {
    my $verification = get_verification();
    
    is $verification->{payment_withdraw}->('payment_withdraw')->{subject}, "Verify your withdrawal request - $website_name", 'Payment Withdraw subject';
    is $verification->{payment_withdraw}->('payment_withdraw')->{message}, get_verification_message('payment_withdraw'), 'Payment Withdraw with token';
    is $verification->{payment_withdraw}->('payment_agent_withdraw')->{message}, get_verification_message('payment_agent_withdraw'), 'Payment Agent Withdraw with token';

    $verification = get_verification(1);
    
    is $verification->{payment_withdraw}->('payment_withdraw')->{message}, get_verification_message('payment_withdraw', 'payment_withdraw'), 'Payment Withdraw with verification URI';
    is $verification->{payment_withdraw}->('payment_agent_withdraw')->{message}, get_verification_message('payment_agent_withdraw', 'payment_agent_withdraw'), 'Payment Agent Withdraw with verification URI';
};
...
subtest 'Password Reset Verification' => sub {
    ...
    is $verification->{reset_password}->()->{message}, get_verification_message('reset_password', 'reset_password'), 'Password Reset with verification URI';
};
