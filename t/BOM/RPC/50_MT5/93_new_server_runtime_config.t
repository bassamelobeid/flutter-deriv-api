...
subtest 'create mt5 client with different currency' => sub {
    subtest 'define new trade server' => sub {
        ...
        note("set app_config->system->mt5->new_trade_server('{\"real\": {\"p02_ts02\":{\"ng\":\"p02_ts02\"}}}')");
        $app_config->system->mt5->new_trade_server('{"real": {"p02_ts02":{"ng":"p02_ts02"}}}');
        note("set app_config->system->mt5->real->p01_ts02->all(0)");
        $app_config->system->mt5->suspend->real->p01_ts02->all(0);
        note("set app_config->system->mt5->real->p02_ts02->all(1)");
        $app_config->system->mt5->suspend->real->p02_ts02->all(1);
