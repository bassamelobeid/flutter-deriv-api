...
subtest 'error check' => sub {
    throws_ok { BOM::MarketData::EarlyClose->new(recorded_date => $now) } qr/required/, 'throws error if not enough argument to create a early close calendar';
    throws_ok { BOM::MarketData::EarlyClose->new(calendar => {}) } qr/required/, 'throws error if not enough argument to create a early close calendar';
    lives_ok { BOM::MarketData::EarlyClose->new(recorded_date => $now, calendar => {}) } 'creates a early close object if all args are present';
};

subtest 'save and retrieve early close dates' => sub {
    lives_ok {
        my $ec = BOM::MarketData::EarlyClose->new(
            recorded_date => $now,
            calendar => {
                $now->epoch => {
                    "18:00" => ['FOREX'],
        ...
        $ec = BOM::MarketData::EarlyClose->new(
            recorded_date => $now,
            calendar => {
                $now->epoch => {
                    "18:00" => ['ASX'],
    ...
    lives_ok {
        my $early_closes = BOM::MarketData::EarlyClose::get_early_closes_for('FOREX');
        ...
        $early_closes = BOM::MarketData::EarlyClose::get_early_closes_for('ASX');
...
subtest 'save and retrieve early closes in history' => sub {
    ...
    my $h = BOM::MarketData::EarlyClose::get_early_closes_for('EURONEXT', $yesterday);
    ok $h->{$now->truncate_to_day->epoch}, '18:00';
    $h = BOM::MarketData::EarlyClose::get_early_closes_for('EURONEXT', $yesterday->minus_time_interval('1d'));
