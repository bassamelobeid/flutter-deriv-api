...
subtest 'error check' => sub {
    throws_ok { BOM::MarketData::PartialTrading->new(recorded_date => $now) } qr/required/,
        'throws error if not enough argument to create a early close calendar';
    throws_ok { BOM::MarketData::PartialTrading->new(calendar => {}) } qr/required/,
        'throws error if not enough argument to create a early close calendar';
    lives_ok { BOM::MarketData::PartialTrading->new(type => 'early_closes', recorded_date => $now, calendar => {}) }
