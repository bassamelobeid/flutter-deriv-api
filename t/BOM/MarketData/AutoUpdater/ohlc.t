...
subtest everything => sub {
    ...
    lives_ok {
        $updater->run();
        like(
            $updater->report->{error}->[0],
            qr/File does not exist/,
            'returns correct message if file does not exist'
        );
    }
    'file does not exist';
    ...
    lives_ok {
        ...
        like(
            $updater->report->{error}->[0],
            qr/files in Bloomberg seems too big/,
            'returns correct message if file is too big'
        );
    }
    'file is too big';
    ...
    lives_ok {
        $updater->run();
        like( $updater->report->{error}->[0],
            qr/empty/, 'added file name to empty list for empty ohlc file' );
    }
    'file is empty';
    ...
    lives_ok {
        ...
        like(
            $updater->report->{error}->[0],
            qr/unregconized bloomberg symbol/i,
            'added invalid Bloomberg symbol to skipped list'
        );
        ok !$updater->report->{N225}->{success},
          'N225 failed to update because of invalid date';
        like(
            $updater->report->{N225}->{reason},
            qr/Incorrect date/,
            'added incorrect date symbol to skipped list'
        );
    }
