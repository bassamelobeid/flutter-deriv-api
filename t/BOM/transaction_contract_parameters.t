...
sub check_emitted_event {
    ...
    my $txns     = $args{event} eq 'sell' ? $args{sell_txns} : $args{buy_txns};

    my %emitted_hash = map { $_->[1]->{contract_id} => $_ } @emitted;

    if ($args{event} eq 'sell' and not $txns) {
        my $mapper = BOM::Database::DataMapper::FinancialMarketBet->new({
            broker_code => $client->broker_code,
            operation   => 'replica'
        });
        my $buy_txns = $args{buy_txns};
        for my $buy (@$buy_txns) {
            my $details   = $mapper->get_contract_details_with_transaction_ids($buy->{fmb}->{id});
            my $sell_trxn = BOM::Database::AutoGenerated::Rose::Transaction::Manager->get_transaction(
                query => [
                    financial_market_bet_id => $buy->{fmb}->{id},
                    action_type             => 'sell'
                ],
                db => $client->db,
            )->[0];
    ...
    is scalar @emitted, scalar @$txns, 'transaction and event lists have the same length';
    ...
    for my $i (0 .. @$txns - 1) {
        ...
        is_deeply $payload, $expected_payload, 'event proporties match the fmb record';
    }
}
...
#        });
#
#        my (@expired_txnids, @expired_fmbids, @unexpired_fmbids, @expired_details, @unexpired_details);
...
#            push @expired_details,
#                {
#                txn => $txn->transaction_details,
#                fmb => $txn->contract_details
#                };
#        }
#
#        check_emitted_event(
#            event    => 'buy',
#            buy_txns => \@expired_details,
#            contract => $contract_expired,
#            client   => $cl
#        );
#        undef @emitted;
