...
$t = $t->get_ok("/authorize?app_id=XXX&redirect_uri=http://localhost/&response_type=token");
...
$t = $t->get_ok("/authorize?app_id=binarycom&redirect_uri=http://localhost/&response_type=token");
...
# confirm_scopes
$t = $t->get_ok("/authorize?app_id=binarycom&redirect_uri=http://localhost/&response_type=token")->content_like(qr/confirm_scopes/);

my $csrftoken = $t->tx->res->dom->at('input[name=csrftoken]')->val;
ok $csrftoken, 'csrftoken is there';

$t->post_ok(
    "/authorize?app_id=binarycom&redirect_uri=http://localhost/&response_type=token" => form => {
        confirm_scopes => 1,
        csrftoken      => $csrftoken
    });
...
$t = $t->get_ok("/authorize?app_id=binarycom&redirect_uri=http://localhost/&scope=trade&response_type=token")->content_like(qr/confirm_scopes/);
