...
subtest 'produce_batch_contract - error check' => sub {
    ...
    is $ask_prices->{RANGE}->{'100.250-98.750'}->{ask_price},    2.74, 'correct ask price';
    is $ask_prices->{UPORDOWN}->{'100.250-98.750'}->{ask_price}, 7.99, 'correct ask price';
    ...
    isa_ok $error, 'BOM::Product::Exception';
    is $error->message_to_client->[0], 'Invalid barrier (Double barrier input is expected).';
