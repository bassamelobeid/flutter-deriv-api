...
subtest 'tick expiry' => sub {
    subtest 'PUT - exit tick higher than entry tick will be settled as loss' => sub {
    ...
    subtest 'PUT - exit tick equal to entry tick will be settled as loss' => sub {
    ...
    subtest 'PUT - exit tick lower to entry tick will be settled as win' => sub {
...
subtest 'intraday' => sub {
    subtest 'PUT - not expired 1 second before expiry' => sub {
        my $c = produce_contract({%$args, date_pricing => $args->{date_start}->plus_time_interval($args->{duration})->minus_time_interval('1s')});
        ok !$c->is_expired, 'not expired';
    };

    subtest 'PUT - date_pricing == date_expiry with inconsistent exit tick' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks([100, $now->epoch + 1, $symbol], [99, $now->epoch + 2, $symbol]);
    ...
    subtest 'PUT - date_pricing == date_expiry with consistent exit tick lower than entry spot' => sub {
    ...
    subtest 'PUT - date_pricing == date_expiry with consistent exit tick higher than entry spot' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                         $symbol],
...
subtest 'multiday' => sub {
    subtest 'PUT - expired but no OHLC data' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                                                 $symbol],
            [102, $now->epoch + 2,                                                 $symbol],
    ...
    subtest 'PUT - expired with OHLC data' => sub {
