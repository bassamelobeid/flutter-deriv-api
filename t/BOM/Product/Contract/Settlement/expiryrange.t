...
subtest 'intraday' => sub {
    subtest 'EXPIRYRANGE - not expired 1 second before expiry' => sub {
        my $c = produce_contract({%$args, date_pricing => $args->{date_start}->plus_time_interval($args->{duration})->minus_time_interval('1s')});
        ok !$c->is_expired, 'not expired';
    };

    subtest 'EXPIRYRANGE - date_pricing == date_expiry with inconsistent exit tick' => sub {
    ...
    subtest 'EXPIRYRANGE - date_pricing == date_expiry with consistent exit tick lower than low barrier = zero payout' => sub {
    ...
    subtest 'EXPIRYRANGE - date_pricing == date_expiry with consistent exit tick equals to low barrier = zero payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,   $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGE - date_pricing == date_expiry with consistent exit tick higher than high barrier = zero payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGE - date_pricing == date_expiry with consistent exit tick equals to high barrier = zero payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGE - date_pricing == date_expiry with consistent exit tick in between the high barrier and low barrier = full payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                         $symbol],
...
subtest 'multiday' => sub {
    subtest 'EXPIRYRANGE - expired but no OHLC data' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                                                 $symbol],
    ...
    subtest 'EXPIRYRANGE - expired with OHLC data' => sub {
