...
subtest 'intraday' => sub {
    subtest 'EXPIRYRANGEE - not expired 1 second before expiry' => sub {
        my $c = produce_contract({%$args, date_pricing => $args->{date_start}->plus_time_interval($args->{duration})->minus_time_interval('1s')});
        ok !$c->is_expired, 'not expired';
    };

    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with inconsistent exit tick' => sub {
    ...
    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with consistent exit tick lower than low barrier = zero payout' => sub {
    ...
    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with consistent exit tick equals to low barrier = full payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,   $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with consistent exit tick higher than high barrier = zero payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with consistent exit tick equals to high barrier = full payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1,                         $symbol],
    ...
    subtest 'EXPIRYRANGEE - date_pricing == date_expiry with consistent exit tick in between the high barrier and low barrier = full payout' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                         $symbol],
...
subtest 'multiday' => sub {
    subtest 'EXPIRYRANGEE - expired but no OHLC data' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                                                 $symbol],
    ...
    subtest 'EXPIRYRANGEE - expired with OHLC data' => sub {
        ...
        ok !$c->waiting_for_settlement_tick, 'waiting for settlement tick';
        ok !$c->require_manual_settlement,   'does not require manual settlement';
        is $c->value, $c->payout, 'win - because close in between high barrier and low barrier';
    };
};
