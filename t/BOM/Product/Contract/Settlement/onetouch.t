...
subtest 'tick expiry' => sub {
    subtest 'ONETOUCH - exit tick crosses barrier. Contract will be settled as a win' => sub {
    ...
    subtest 'ONETOUCH - exit tick touches barrier. Contract will be settled as a win' => sub {
    ...
    subtest 'ONETOUCH - tick touches barrier before expiry. Contract will be settled as a win' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks([100, $now->epoch + 1, $symbol], [100.10, $now->epoch + 2, $symbol]);
    ...
    subtest 'ONETOUCH - no tick touches the barrier. Contract will be settled as a loss' => sub {
...
subtest 'intraday' => sub {
    subtest 'ONETOUCH - not expired 1 second before expiry' => sub {
        my $c = produce_contract({%$args, date_pricing => $args->{date_start}->plus_time_interval($args->{duration})->minus_time_interval('1s')});
        ok !$c->is_expired, 'not expired';
    };

    subtest 'ONETOUCH - not ok through expiry' => sub {
        my $mocked_c = Test::MockModule->new('BOM::Product::Contract::Onetouch');
    ...
    subtest 'ONETOUCH - Does not touch the barrier and ok through expiry. Contract will be settled as a loss.' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                         $symbol],
            [99,  $now->epoch + 2,                         $symbol],
...
subtest 'multiday' => sub {
    subtest 'ONETOUCH - expired but no OHLC data' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100, $now->epoch + 1,                                                 $symbol],
    ...
    subtest 'ONETOUCH - expired with OHLC data' => sub {
