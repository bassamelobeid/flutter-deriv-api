...
subtest 'tick expiry' => sub {
    subtest 'RUNHIGH - first tick is lower than entry tick. Contract will be settled as a loss' => sub {
    ...
    subtest 'RUNHIGH - first tick is equals to entry tick. Contract will be settled as a loss' => sub {
    ...
    subtest 'RUNHIGH - second tick is lower than first tick. Contract will be settled as a loss' => sub {
    ...
    subtest 'RUNHIGH - last tick is lower than second last tick. Contract will be settled as a loss' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1, $symbol],
            [100.01, $now->epoch + 2, $symbol],
            [100.02, $now->epoch + 3, $symbol],
            [100.03, $now->epoch + 4, $symbol],
            [100.04, $now->epoch + 5, $symbol],
            [100.03, $now->epoch + 6, $symbol]);
        my $c = produce_contract({%$args, date_pricing => $now->plus_time_interval('10s')});
        is $c->entry_tick->quote, 100,    'entry tick is 100';
        is $c->hit_tick->quote,   100.03, 'hit tick is 100.03';
        ok $c->is_expired,       'contract is expired';
        ok $c->is_valid_to_sell, 'is valid to sell';
        ok !$c->waiting_for_settlement_tick, 'not waiting for settlement tick';
        ok !$c->require_manual_settlement,   'does not require manual settlement';
    ...
    subtest 'RUNHIGH - all ticks are higher than previous tick. Contract will be settled as a win' => sub {
        BOM::Test::Data::Utility::FeedTestDatabase::flush_and_create_ticks(
            [100,    $now->epoch + 1, $symbol],
            [100.01, $now->epoch + 2, $symbol],
            [100.02, $now->epoch + 3, $symbol],
