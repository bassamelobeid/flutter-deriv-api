...
subtest 'submarkets' => sub {
    my @expected = (
        qw(americas amsterdam asia_oceania belgium energy europe_africa otc_index us_otc_stock uk_otc_stock ge_otc_stock au_otc_stock france major_pairs metals middle_east minor_pairs random_daily random_index smart_fx)
    );
    lives_ok {
...
subtest 'underlying symbols' => sub {
    my %expected = (
        ...
        stocks      => [
            qw( USAAPL USAMZN USCT USFB USGE USGOOG USKO USMSFT USPFE USXOM UKBARC UKBATS UKGSK UKHSBA UKVOD DEALV DEBAYER DEDAI DESIE DEVOWGEN AUANZ AUBHP AUCBA AUMQG AUQAN BBABI BBBELG BBGBLB BBKBC BBUCB FPACA FPAI FPAIR FPBN FPBNP FPCA FPCS FPDG FPEDF FPEI FPFP FPGLE FPGSZ FPKER FPMC FPOR FPORA FPRI FPRNO FPSAF FPSAN FPSGO FPSU FPVIV NAASML NAHEIA NAINGA NARDSA NAUNA)
        ],
