...
subtest 'vol_proxy and trend proxy' => sub {
    ...
    my $vol_proxy_reference = get_parameter(
        'vol_proxy',
        {
    ...
    my $trend_proxy_reference = get_parameter(
        'trend_proxy',
        {
            underlying   => $underlying,
            date_pricing => $now
    ...
    $vol_proxy_reference = get_parameter(
        'vol_proxy',
        {
            underlying   => $underlying,
            date_pricing => $now
    ...
    $trend_proxy_reference = get_parameter(
        'trend_proxy',
        {
            underlying   => $underlying,
            date_pricing => $now
...
subtest 'economic_events' => sub {
    ...
    my @eco = get_parameter(
        'economic_events',
        {
            underlying => $underlying,
            start      => $now,
            end        => $now->plus_time_interval('1h')});
