...
BOM::Test::Data::Utility::UnitTestMarketData::create_doc('volsurface_delta', {symbol => 'frxUSDJPY', recorded_date => $efficient_time});
BOM::Test::Data::Utility::UnitTestMarketData::create_doc('currency', {symbol => $_, recorded_date => $efficient_time}) for qw(USD JPY JPY-USD);
...
$mock->mock('retrieve', sub {[ map {{epoch => $_, quote => 10}} (0..5) ]});
...
subtest 'inefficient craziness' => sub {
    ...
    ok !$c->pricing_engine->risk_markup->peek_amount('intraday_eod_markup'), '10% eod markup not added for ATM';
