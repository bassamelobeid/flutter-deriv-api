...

{
    ...
    my $dbh = BOM::Platform::Chronicle::_dbic()->dbh;
    isa_ok($dbh, 'DBI::db');
    ...
    is(refaddr(BOM::Platform::Chronicle::_dbic()->dbh), refaddr($dbh), 'have same ref when calling _dbh again');
    ok(BOM::Platform::Chronicle::_dbic()->dbh->ping, 'can still ping');
    # Note that this would not be valid on win32 where all refaddrs
    # change after a "fork" anyway.
    my $addr = refaddr($dbh);
    if (my $pid = fork // die "fork failed - $!") {
        # Parent
        is(refaddr(BOM::Platform::Chronicle::_dbic()->dbh), $addr, 'refaddr still the same in parent after fork');
        ...
        isnt(refaddr(my $child_dbh = BOM::Platform::Chronicle::_dbic()->dbh), $addr, 'refaddr changes in a fork');
        isa_ok($child_dbh, 'DBI::db');
        is(refaddr(BOM::Platform::Chronicle::_dbic()->dbh), refaddr($child_dbh), 'but subsequent calls get the same object');
        ...
        ok(BOM::Platform::Chronicle::_dbic()->dbh->ping, 'can ping the second handle');
        exit 0;
    }
}
done_testing;
