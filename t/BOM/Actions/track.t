...
subtest 'General event validation - filtering by brand' => sub {
    ...
    test_segment_customer($customer, $test_client, '', $test_client->date_joined);
    my $expected_args = {
        'context' => {
            'active' => 1,
            'app'    => {'name' => 'deriv'},
...
subtest 'Tracking blocked events' => sub {
    my $blocked_events = {svg => ['login']};
    $mock_brands->mock(
        tracking_blocked_events => sub {
            my $self = shift;
            return $blocked_events;
    ...
    is BOM::Event::Services::Track::track_event(
        ...
        'Empty response when calling if event is blocked for the proker code';
    is @identify_args, 0, 'Segment identify is not invoked';
    is @track_args,    0, 'Segment track is not invoked';

    $blocked_events = {virtual => ['login']};

    ok BOM::Event::Services::Track::track_event(
        event   => 'login',
        loginid => $test_client->loginid
    )->get, 'SVG is not blocked anymore';
    is @identify_args, 0, 'Segment identify is not invoked';
    ok @track_args, 'Segment track is invoked';

    $mock_brands->unmock('tracking_blocked_events');
};
