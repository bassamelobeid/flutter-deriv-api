...
subtest "check btc fraud address in TP" => sub {
    my $client = create_client();
    $client->set_default_account('BTC');
    my $helper = BOM::CTC::Helper->new(client => $client);
...
subtest "check eth fraud addresses in TP" => sub {
    my $client = create_client();
    $client->set_default_account('ETH');
    my $helper = BOM::CTC::Helper->new(client => $client);
    ...
    $mock_eth_third_party->mock(
        'tp_fraud_address' => async sub {
            my ($self, $address) = @_;
            if ($address eq "ETH_FRAUD_ADDRESS") {
                return {
                    ...
                    "result"  => {
                        ...
                        "entries" => [{
                                "id"          => "afcdbd",
                                "name"        => "ethcomprules.cx",
                                "type"        => "scam",
                                "url"         => "https=>//ethcomprules.cx",
                                "hostname"    => "ethcomprules.cx",
                                "featured"    => 0,
                                "path"        => "/*",
                                "category"    => "Scamming",
    ...
    my $mock_cryptoSubscription = Test::MockModule->new('BOM::CTC::Currency::ETH');
    my $dd_metric_counter       = 0;
    my $dd_metric_tags;
    my $dd_metric_name;
    my $expected_dd_metric_name = 'bom_cryptocurrency.subscription.fraud_address';
    my $expected_dd_metric_tags = {tags => ['currency:ETH', 'status:failed', 'thirdparty:cryptoscamdb']};
    $mock_cryptoSubscription->mock(
        stats_inc => sub {
            ($dd_metric_name, $dd_metric_tags) = @_;
            $dd_metric_counter++ if ($dd_metric_name eq $expected_dd_metric_name);
        });

    $mock_cryptoSubscription->mock(
        get_valid_address => sub {
            my ($self, $address) = @_;
            return {
                "address" => $address,
                "check"   => 1,
    ...
    is $dd_metric_counter, 1, 'increase datadog metric when there is a problem in communicating with tp API';
    is_deeply $dd_metric_tags, $expected_dd_metric_tags, 'correct dd tags';

    $response = BOM::Event::Actions::CryptoSubscription::fraud_address({
        address       => $eth_fraud_address,
        currency_code => "ETH"
    });
    is $response, '1', "address found on the TP";

    $mock_cryptoSubscription->unmock_all();

    my $rows = $dbic->run(
        fixup => sub {
            my $sth = $_->prepare(q{SELECT * FROM payment.blacklist_addresses where address = (?)});
            $sth->execute($eth_fraud_address);
            return $sth->fetchall_arrayref({});
        });

    is $rows->@*, 1, "Address was correctly inserted in the database";
    is @$rows[0]->{address_report_count}, 2, "Address report count has been inserted correctly in the database";
