...
subtest 'client_transfer event' => sub {
    ...
    my $user = BOM::User->create(
        email          => 'client_transfer@test.com',
        password       => "hello",
        email_verified => 1,
    );
    ...
    my $handler = BOM::Event::Process::get_action_mappings()->{client_transfer};

    my $args = {
        loginid_from      => $client_usd->loginid,
        loginid_to        => $client_btc->loginid,
        from_currency     => 'USD',
        to_currency       => 'BTC',
        is_mt5_login_from => 0,
        is_mt5_login_to   => 0,
    ...
    sub test_status {
        my ($locked_clients, $non_locked_clients, $title) = @_;
        subtest $title => sub {
            # force to reload status from db
            $_->{status} = undef for (@$locked_clients, @$non_locked_clients);

            for (@$locked_clients) {
                ok $_->status->withdrawal_locked, $_->loginid . ' is withdrawal locked';
                $_->status->clear_withdrawal_locked;
            }

            for (@$non_locked_clients) {
                ok !$_->status->withdrawal_locked, $_->loginid . ' is not withdrawal locked';
            }
            }
    }
    ...
    $args->{amount}       = 1000;
    $args->{lc_short}     = 'svg';
    $args->{loginid_from} = $client_vr->loginid;
    $handler->($args)->get;
    test_status([], [$client_vr, $client_usd, $client_btc], 'Nothing happens with virtual loginid');

    $args->{amount}       = 1000;
    $args->{lc_short}     = 'malta';
    $args->{loginid_from} = $client_mlt->loginid;
    $handler->($args)->get;
    test_status([], [$client_vr, $client_usd, $client_btc], 'Nothing happens for non-svg landing companies');

    $args->{lc_short}     = 'svg';
    $args->{loginid_from} = $client_usd->loginid;
    $args->{amount}       = 1000;
    $handler->($args)->get;
    test_status([$client_usd, $client_btc], [$client_vr], 'Real accounts are locked with amount 1000');

    $args->{amount} = 999;
    $handler->($args)->get;
    test_status([], [$client_vr, $client_usd, $client_btc], 'Nothing is locked with amount 999');

    $args->{amount} = 1;
    $handler->($args)->get;
    test_status([$client_usd, $client_btc], [$client_vr], 'Real accounts are locked with amount 999+1');
