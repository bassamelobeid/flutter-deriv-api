...
subtest 'Client withdraw money via payment agent' => sub {
    ...
    lives_ok {
        ...
        $amount_data = $clientdb->fetchall_arrayref('select * from payment.get_today_payment_agent_withdrawal_sum_count(?)', [$pa_client->loginid]);
    ...
    lives_ok {
        ...
        $amount_data = $clientdb->fetchall_arrayref('select * from payment.get_today_payment_agent_withdrawal_sum_count(?)', [$client->loginid]);
        ($total_withdrawal, $withdrawal_count) = ($amount_data->[0]->{amount}, $amount_data->[0]->{count});
    }
