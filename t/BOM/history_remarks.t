...
subtest 'doughflow' => sub {
    my $client = BOM::Test::Data::Utility::UnitTestDatabase::create_client({broker_code => 'CR', email => 'doughflow@binary.com'});
    ...
    $client->payment_doughflow(%params, amount => 10, payment_fee => 1, payment_method => 'MegaPay', trace_id => 101);
    ...
    $client->payment_doughflow(%params, amount => 10, payment_fee => 1, payment_processor => 'UltraPay', trace_id => 102);
    ...
    $client->payment_doughflow(%params, amount => -10, payment_method => 'NicePay', trace_id => 103);
    @res = get_remarks($client);
    is $res[0], 'NicePay trace ID 103', 'Withdrawal';
    
    $client->payment_doughflow(%params, transaction_type => 'withdrawal_reversal', amount => 10, payment_fee => -1, payment_method => 'BigPay', trace_id => 104);
    @res = get_remarks($client);
...
subtest 'crypto' => sub {
    my $client = BOM::Test::Data::Utility::UnitTestDatabase::create_client({broker_code => 'CR', email => 'crypto@binary.com'});
    $client->account('BTC');
    BOM::User->create(
        email    => $client->email,
        password => 'test'
...
subtest 'mt5 transfers' => sub {
    my $client = BOM::Test::Data::Utility::UnitTestDatabase::create_client({broker_code => 'CR', email => 'mt5@binary.com'});
...
subtest 'transfer between accounts' => sub {
    my $client_usd = BOM::Test::Data::Utility::UnitTestDatabase::create_client({broker_code => 'CR', email => 'tba@binary.com'});
    ...
    my $client_mf = BOM::Test::Data::Utility::UnitTestDatabase::create_client({broker_code => 'MF', email => $client_usd->email});
    ...
    @res = get_remarks($client_usd);
    is $res[0], 'Account transfer from '.$client_btc->loginid.'. Includes the minimum transfer fee of 0.00200000 BTC.', 'Transfer back with min fee';
    
    @res = get_remarks($client_btc);
    is $res[0], 'Account transfer to '.$client_usd->loginid.'. Includes the minimum transfer fee of 0.00200000 BTC.', 'Transfer back with min fee';   
...
subtest 'p2p' => sub {
    ...
    subtest 'sell ads' => sub {
        ...
        is $res[0], 'P2P order ' . $order->{id} . ' created by cody (' . $client->loginid . ') - seller funds held', 'order create remark for advertiser';
        ...
        @res = get_remarks($client);
        is $res[0], 'P2P order ' . $order->{id} . ' completed - funds received from andy (' . $advertiser->loginid . ')',
    ...
    subtest 'buy ads' => sub {
        ...
        @res = get_remarks($advertiser);
        is $res[0], 'P2P order ' . $order->{id} . ' completed - funds received from cody (' . $client->loginid . ')',
            'payment remark for advertiser';
