...
BOM::System::Chronicle->_redis_write->publish( 'FEED::R_50',
    'R_50;1447998048;443.6823;' );
$t->message_ok;
...
$t = $t->send_ok( { json => { forget => $proposal->{proposal}->{id} } } )
    ->message_ok;
...
$t = $t->send_ok(
    {   json => {
            ...
            contract_id =>
                $portfolio->{portfolio}->{contracts}->[0]->{contract_id},
        } } );
$t = $t->message_ok;
my $res = decode_json( $t->message->[1] );
