...
subtest 'append_failure' => sub {
    ...
    lives_ok { $result->append_failure('rule 1', {error_code => 'ErrorCode1'}) } 'failure 1 appended';
    is $result->has_failure, 1, 'has_failure value changed to 1 correctly';
    is_deeply $result->failed_rules, {'rule 1' => {error_code => 'ErrorCode1'}}, 'failed_rule is updated correctly';
    ...
    lives_ok { $result->append_failure('rule 2', {error_code => 'ErrorCode2', other => 'value'}) } 'failure 2 appended';
    is $result->has_failure, 1, 'has_failure is still 1 correctly';
    is_deeply $result->failed_rules,
        {
        'rule 1' => {error_code => 'ErrorCode1'},
        'rule 2' => {
...
subtest 'merge test 1' => sub {
    ...
    lives_ok { $result_1->append_failure('rule 1', {error_code => 'ErrorCode1'}) } 'failure 1 appended';
    lives_ok { $result_1->append_success('rule 1') } 'success 1 appended';
    lives_ok { $result_1->append_failure('rule 2', {error_code => 'ErrorCode2'}) } 'failure 2 appended';

    my $result_2 = BOM::Rules::Result->new();
    lives_ok { $result_2->append_success('rule 2') } 'success 2 appended';
    lives_ok { $result_2->append_failure('rule 3', {error_code => 'ErrorCode3'}) } 'failure 3 appended';
    ...
    is_deeply $result_merged->failed_rules,
        {
        'rule 1' => {error_code => 'ErrorCode1'},
        'rule 2' => {error_code => 'ErrorCode2'},
        'rule 3' => {error_code => 'ErrorCode3'},
        },
...
subtest 'merge test 2' => sub {
    ...
    my $result_2 = BOM::Rules::Result->new();
    lives_ok { $result_2->append_failure('rule 1', {error_code => 'ErrorCode1', other => 'value'}) } 'failure 1 appended';
