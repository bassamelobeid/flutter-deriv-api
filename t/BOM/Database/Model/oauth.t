...
my $is_confirmed = $m->is_scope_confirmed($test_clientid, $test_loginid, 'user', 'trade');
is $is_confirmed, 0, 'not confirmed';

ok $m->confirm_scope($test_clientid, $test_loginid, 'user', 'trade'), 'confirm scope';
$is_confirmed = $m->is_scope_confirmed($test_clientid, $test_loginid, 'user', 'trade');
is $is_confirmed, 1, 'confirmed after confirm_scope';

# create then verify
my $code = $m->store_auth_code($test_clientid, $test_loginid, 'user', 'trade');
...
## try access_token
my ($access_token, $refresh_token) = $m->store_access_token($test_clientid, $test_loginid, @scope_ids);
ok $access_token;
ok $refresh_token;
