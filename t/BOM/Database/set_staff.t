...
sub get_audit {
    ...
    return $clientdb->db->dbic->run(sub {
...
], 'audit records after creating a new client';
...
{
    BOM::Database::ClientDB->new({broker_code => 'CR'})->db->dbic->run(sub {
...
is_deeply get_audit($loginid), [
    ...
    [qw!UPDATE xx 192.168.12.25/32!],
    [qw!UPDATE system 127.0.0.1/32!],
    COMPATIBLE_MODE ? [qw!UPDATE ?!, undef] : [qw!UPDATE mt 192.168.12.26/32!],
