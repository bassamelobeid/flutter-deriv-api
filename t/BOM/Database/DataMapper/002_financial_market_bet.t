...
my $fmb_mapper;
lives_ok {
    $fmb_mapper = BOM::Database::DataMapper::FinancialMarketBet->new({
        client_loginid => 'CR0021',
        currency_code  => 'USD',
    });
}
...
cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 0, 'check qty open bet = 0');
...
cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 1, 'check qty open bet = 1');
...
cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 2, 'check qty open bet = 1');
...
'buy higher lower relative barrier bet';
cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 3, 'check qty open bet = 2');
...
isnt scalar sell({
    ...
    undef, 'sell 1 bet';
cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 1, 'check qty open bet = 0');
...
subtest 'digits' => sub {
    ...
    foreach my $type (sort keys %type_prediction) {
        subtest $type => sub {
            ...
            'buy';
            cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', 1+$c, 'check qty open bet = 1');
            ...
            isnt scalar sell({
                ...
                undef, 'sell';
            cmp_ok($fmb_mapper->get_open_bets_of_account(), '==', $c, 'check qty open bet = 0');
            $c++;
        };
    }
};
