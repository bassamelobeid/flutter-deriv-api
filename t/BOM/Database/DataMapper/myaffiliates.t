...
cmp_ok($activities->{'MX1001'}->{'withdrawals'},        '==', 160,  'Check if activity withdrawals is correct for myaffiliate');
cmp_ok($activities->{'MX1001'}->{'deposits'},           '==', 6720, 'Check if activity deposits is correct for myaffiliate');
cmp_ok($activities->{'MX1001'}->{'pnl'},                '==', 0,    'Check if activity pnl is correct for myaffiliate');
cmp_ok($activities->{'MX1001'}->{'turnover_others'},    '==', 86,   'Check if activity turnover_others is correct for myaffiliate');
cmp_ok($activities->{'MX1001'}->{'turnover_intradays'}, '==', 8.32, 'Check if turnover_intradays factors is correct for myaffiliate');
