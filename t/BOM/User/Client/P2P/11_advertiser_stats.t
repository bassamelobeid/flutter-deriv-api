...
subtest 'errors' => sub {
   ...
   cmp_deeply( 
       exception { $cli->p2p_advertiser_stats(id=>-1) }, 
       { error_code => 'AdvertiserNotFound' },
      'Advertiser not found'
   );
   
   cmp_deeply( 
       exception { $cli->p2p_advertiser_stats() }, 
       { error_code => 'AdvertiserNotRegistered' },
      'Client not advertiser'
   );
...
subtest 'sell ads' => sub {
   ...
   ($client,$order) = BOM::Test::Helper::P2P::create_order(client=>$client, advert_id=>$advert->{id}, amount=>1, balance=>100);
   ...
   ($client,$order) = BOM::Test::Helper::P2P::create_order(client=>$client, advert_id=>$advert->{id}, amount=>1);
   cmp_deeply($advertiser->p2p_advertiser_stats, $stats_adv, 'advertiser stats after order created');
   cmp_deeply($client->p2p_advertiser_stats, $stats_cli, 'client stats after order created');
   ...
   ($client,$order) = BOM::Test::Helper::P2P::create_order(client=>$client, advert_id=>$advert->{id}, amount=>2);
   $client->p2p_order_confirm(id=>$order->{id});
...
subtest 'buy ads' => sub {
    ...
    ($advertiser, $advert) = BOM::Test::Helper::P2P::create_advert(client=>$advertiser, type =>'buy');
    ($client,$order) = BOM::Test::Helper::P2P::create_order(client=>$client, advert_id=>$advert->{id}, amount=>1);
    cmp_deeply($advertiser->p2p_advertiser_stats, $stats_adv, 'advertiser stats after order created');
    cmp_deeply($client->p2p_advertiser_stats, $stats_cli, 'client stats after order created');
