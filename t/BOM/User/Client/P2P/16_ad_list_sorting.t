...
my (undef, $buy_ad1) = BOM::Test::Helper::P2P::create_advert(client=>$adv1, type=>'buy', rate=>100);
my (undef, $buy_ad2) = BOM::Test::Helper::P2P::create_advert(client=>$adv2, type=>'buy', rate=>99);
my (undef, $sell_ad1) = BOM::Test::Helper::P2P::create_advert(client=>$adv1, type=>'sell', rate=>100);
my (undef, $sell_ad2) = BOM::Test::Helper::P2P::create_advert(client=>$adv2, type=>'sell', rate=>101);
...
my $order = $adv1->p2p_order_create(advert_id => $sell_ad2->{id}, amount=>10);
$adv1->p2p_order_cancel(id=>$order->{id});

$order = $adv2->p2p_order_create(advert_id => $buy_ad1->{id}, amount=>11, contact_info=>'x', payment_info=>'x');
$adv1->p2p_order_cancel(id=>$order->{id});

$order = $adv2->p2p_order_create(advert_id => $sell_ad1->{id}, amount=>10);
$adv2->p2p_order_confirm(id=>$order->{id});
$adv1->p2p_order_confirm(id=>$order->{id});

$order = $adv1->p2p_order_create(advert_id => $buy_ad2->{id}, amount=>11, contact_info=>'x', payment_info=>'x');
$adv2->p2p_order_confirm(id=>$order->{id});
$adv1->p2p_order_confirm(id=>$order->{id});

my @ids = map { $_->{id} } $adv1->p2p_advert_list(type=>'buy', sort_by =>'rate')->@*;
cmp_deeply(\@ids, [$buy_ad1->{id}, $buy_ad2->{id}], 'highest rate first for buy ads');

@ids = map { $_->{id} } $adv1->p2p_advert_list(type=>'buy', sort_by =>'completion')->@*;
cmp_deeply(\@ids, [$buy_ad2->{id}, $buy_ad1->{id}], 'sort buy ads by completion');

@ids = map { $_->{id} } $adv1->p2p_advert_list(type=>'sell', sort_by =>'rate')->@*;
cmp_deeply(\@ids, [$sell_ad1->{id}, $sell_ad2->{id}], 'lowest rate first for buy ads');

@ids = map { $_->{id} } $adv1->p2p_advert_list(type=>'sell', sort_by =>'completion')->@*;
