...
subtest 'sync_status_actions' => sub {
    ...
    $status_update_mock->mock(
        ...
        sub {
            ...
            return +{
                ...
                user        => 1,              # in the script this will be BOM::User object
                cr_currency => 'USD',
                bom_loginid => 'CR00000000'    # in the script it will be $client->loginid
            };
        });
    ...
    $status_update_mock->mock(
        'gather_users',
        sub {
            ['MTR100000000', '10000000', $creation_stamp->db_timestamp, 'none', 'mt5', 'real', 'USD', $bvi_group_attributes];
        });
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    my $verification_status = BOM::MT5::Script::StatusUpdate->new;
    $verification_status->sync_status_actions();
    is $to_status, 'verification_pending', 'poi pending, poa pending => verification_pending';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'verification_pending', 'poi pending, poa verified => verification_pending';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'poa_pending', 'poi verified, poa pending => poa_pending';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'poa_failed', 'poi verified, poa failed => poa_failed';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'poa_failed', 'poi verified, poa rejected => poa_failed';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'proof_failed', 'poi failed, poa failed => proof_failed';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'verification_pending', 'within grace period poi pending, poa failed => verification_pending';

    $status_update_mock->mock(
        'gather_users',
        sub {
            [
                'MTR100000000', '10000000', $creation_stamp->minus_time_interval('20d')->db_timestamp,
                'none', 'mt5', 'real', 'USD', $bvi_group_attributes
            ];
        });

    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, 'poa_failed', 'out of grace period poi pending, poa failed => poa_failed';
    ...
    $to_status = 'not_touched';
    $verification_status->sync_status_actions();
    is $to_status, 'not_touched', 'the status is already the correct one, no need to update';
    ...
    $to_status = 'Looks like it didnt trigger the status update';
    $verification_status->sync_status_actions();
    is $to_status, undef, 'poi verified and poa verified, the status shoud be changed to empty';
