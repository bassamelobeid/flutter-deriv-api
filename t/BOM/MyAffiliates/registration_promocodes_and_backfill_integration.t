...
subtest "Sanity check" => sub {
    ...
    is(BOM::MyAffiliates->new->is_subordinate_affiliate(2),                                     undef, "Affiliate 2 is NOT subordinate");
    is(BOM::MyAffiliates->new->get_affiliate_id_from_token('PQ4YXsO2q5nKto_EPcZApGNd7ZgqdRLk'), 2,     "Affiliate 2 token");

    is(BOM::MyAffiliates->new->is_subordinate_affiliate(6),                                     undef, "Affiliate 6 is NOT subordinate");
    is(BOM::MyAffiliates->new->get_affiliate_id_from_token('jGZUKO3JWgyVAv0U_Fv2nVOqZLGcUW5p'), 6,     "Affiliate 6 token");

    # subordinate affiliates and tokens
    is(BOM::MyAffiliates->new->is_subordinate_affiliate(13),                                    1,  "Affiliate 13 is subordinate");
    is(BOM::MyAffiliates->new->get_affiliate_id_from_token('k7a3BtGf-EjKto_EPcZApGNd7ZgqdRLk'), 13, "Affiliate 13 token");

    is(BOM::MyAffiliates->new->is_subordinate_affiliate(274),                                   1,   "Affiliate 274 is subordinate");
    is(BOM::MyAffiliates->new->get_affiliate_id_from_token('7O0pu16F5bbKto_EPcZApGNd7ZgqdRLk'), 274, "Affiliate 274 token");
...
subtest "promocode processing rules: handling usage of promocode and it's updating of tokens" => sub {
    ...
    subtest "-funded +token -> promo => promo" => sub {
        ...
        $is_updated_by_backfill = grep { my $l = $client->loginid; /^$l:/ } BOM::MyAffiliates::BackfillManager->new->backfill_promo_codes;
        ...
        foreach my $initial_token ('7O0pu16F5bbKto_EPcZApGNd7ZgqdRLk', 'jGZUKO3JWgyVAv0U_Fv2nVOqZLGcUW5p', undef) {
            foreach my $promocode ('BOM2009', '0013F10') {
                subtest "using: token => " . ($initial_token or 'undef') . ", promocode => $promocode" => sub {
                    ...
                    my $is_updated_by_backfill =
                        grep { my $l = $client->loginid; /^$l:/ } BOM::MyAffiliates::BackfillManager->new->backfill_promo_codes;
                    is $is_updated_by_backfill, 0, "Backfill reports that it didn't update our client";
...
subtest 'Marketing campaign: Client registers with Affiliate token from cookie and types a subordinate promocode' => sub {
    ...
    my $is_updated_by_backfill = grep { my $l = $client->loginid; /^$l:/ } BOM::MyAffiliates::BackfillManager->new->backfill_promo_codes;
    is $is_updated_by_backfill, 1, "Backfill reports that it updated our client";
...
subtest 'backfill pending' => sub {
    ...
    is(BOM::MyAffiliates::BackfillManager->new->is_backfill_pending, 0, "No promocodes to be backfilled");
    ...
    is(BOM::MyAffiliates::BackfillManager->new->is_backfill_pending, 0, "Still no promocode pending to be backfilled");
    ...
    is(BOM::MyAffiliates::BackfillManager->new->is_backfill_pending, 0, "No promocodes pending to be backfilled");
};
