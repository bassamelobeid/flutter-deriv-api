...
subtest 'positive p2p and reversible, balance more than p2p+reversible' => sub {
    ...
    cmp_ok $advertiser->p2p_withdrawable_balance, '==', $advertiser->{client}->account->balance, 'full balance availble when country is banned';
    ...
    $advertiser->{client}->payment_doughflow(
...
subtest 'transfers' => sub {
    ...
    $advertiser->{client}->user->add_client($client_usdc);
    ...
    $advertiser->{client}->payment_account_transfer(
...
sub check_total {
    my $client = shift;
    cmp_ok $client->p2p_balance + $client->p2p_withdrawable_balance, '>=', $client->{client}->account->balance,
