...
subtest 'Buy adverts' => sub {
    ...
    BOM::Test::Helper::Client::top_up($client->{client}, $client->{client}->currency, $amount);
    ...
    ok($escrow->account->balance == $order_amount,           'Money is deposited to Escrow account');
    ok($client->{client}->account->balance == $amount - $order_amount, 'Money is withdrawn from Client account');
...
subtest 'Seller with empty balance tries to place an "sell" order' => sub {
    ...
    cmp_ok $buyer->{client}->account->balance, '==', 0, "The seller's balance is 0";
...
subtest 'Buyer tries to place a "buy" order with an amount that exceeds the advertiser balance' => sub {
    ...
    BOM::Test::Helper::Client::top_up($advertiser->{client}, $advertiser->{client}->currency, -50);
