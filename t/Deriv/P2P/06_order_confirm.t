...
for my $test_case (@test_cases) {
    ...
    subtest $test_case->{test_name} => sub {
        ...
        cmp_ok($client->{client}->account->balance,     '==', $test_case->{client}{before},     'Client balance is correct');
        ...
        cmp_ok($client->{client}->account->balance,     '==', $test_case->{client}{after},     'Client balance is correct');
...
subtest 'race condition' => sub {
    ...
    cmp_deeply(
        exception { $client->p2p_order_confirm(id => $order->{id}) },
        {error_code => 'OrderConfirmCompleted'},
        'order is not confirmable and caught by db and correct eror returned'
    );
