...
subtest 'get_mf_enable_countries' => sub {
    my $mocked_mf_enable_countries_config;
    my $mocked_global_revision = 1;
    ...
    $mocked_instance->mock("app_config" => sub { return $mocked_app_config });
    $mocked_app_config->mock("get"             => sub { $mocked_mf_enable_countries_config });
    $mocked_app_config->mock("global_revision" => sub { $mocked_global_revision });

    $mocked_mf_enable_countries_config = '{"enable": ["ec","za"]}';
    my $expected = {
        enable   => ["ec", "za"],
        revision => $mocked_global_revision
    };
    cmp_deeply(
        BOM::Config::Compliance->get_mf_enable_country_list(),
        {
            enable   => [qw/ec za/],
            revision => $mocked_global_revision
        },
        "Correct countries for mf countries list"
    );
};

subtest 'get_financial_company' => sub {
    my $mocked_mf_enable_countries_config;
    my $mocked_global_revision = 1;
    my $mocked_instance        = Test::MockObject->new(BOM::Config::Runtime->instance);
    my $mocked_app_config      = Test::MockObject->new();
    ...
    cmp_deeply(BOM::Config::Compliance->get_financial_company('za'), 'maltainvest', 'Correct financial comapany for South Africa');

    $mocked_mf_enable_countries_config = '{"enable": []}';

    cmp_deeply(BOM::Config::Compliance->get_financial_company('de'), 'maltainvest', 'Correct financial comapany for Germany');

    cmp_deeply(BOM::Config::Compliance->get_financial_company('id'), 'svg', 'Correct financial comapany for Indonesia');
};
