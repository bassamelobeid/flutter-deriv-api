...
subtest 'items to reprocess' => sub {
    ...
    $log->contains_ok(qr/Reprocessing event 'very_old_event' from 'TEST_STREAM', attempt # 2\/5/, "The warning message is returned correctly");
...
subtest 'stream process loop' => sub {
    ...
    $log->contains_ok(qr/Event 'event_to_retry' from 'TEST_STREAM' has failed to process. The initial error was : Failed to process the event/,
        "The error message is returned correctly");
    is scalar @acked, 0, 'Item correctly marked as not being acknowledged';
