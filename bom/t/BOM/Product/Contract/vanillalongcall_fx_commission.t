...
subtest 'check spread spot and spread vol is applied (days)' => sub {
    ...
    $fx_per_symbol_config->{spread_spot}->{delta}->{0.5}->{day}->{2} = 0.0006;
    ...
    is $c->bid_probability->adjustments->[0]->[1]->name, 'spread', 'correct adjustment name';

    is sprintf("%.1f", $c->delta),                                             '0.5', 'delta is approximately 0.5';
    is business_days_between($c->date_start, $c->date_expiry, $c->underlying), 2,     '2 days expiry contract';

    is sprintf("%.5f", $c->bid_probability->amount), '0.80250', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount), '0.82055', 'correct ask probability';

    # this contract is also affected due to interpolated spread
    $new_args->{duration} = '1d';
    $c = produce_contract($new_args);
    is business_days_between($c->date_start, $c->date_expiry, $c->underlying), 1, '1 days expiry contract';
    is sprintf("%.5f", $c->bid_probability->amount), '0.62762', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount), '0.64750', 'correct ask probability';
...
subtest 'check spread spot and spread vol is applied (weeks)' => sub {
    ...
    is sprintf("%.5f", $c->bid_probability->amount), '1.58607', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount), '1.68305', 'correct ask probability';

    # this contract is affected due to interpolated spread
    $new_args->{duration} = '10d';
    $c = produce_contract($new_args);
    is weeks_between($c->date_start, $c->date_expiry), 1, '1 week expiry contract';
    is sprintf("%.5f", $c->bid_probability->amount), '1.44812', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount), '1.49416', 'correct ask probability';
...
subtest 'check spread spot and spread vol is applied (edge case delta)' => sub {

    my $new_args = dclone($args);

    $new_args->{barrier} = '126';
    my $c = produce_contract($new_args);
    is sprintf("%.2f", $c->delta),                                             '0.93',    'delta is approximately 0.9';
    is business_days_between($c->date_start, $c->date_expiry, $c->underlying), 2,         '2 days expiry contract';
    is sprintf("%.5f", $c->bid_probability->amount),                           '6.34304', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount),                           '6.34304', 'correct ask probability';

    $fx_per_symbol_config                                            = JSON::MaybeXS::decode_json($fx_per_symbol_config);
    $fx_per_symbol_config->{spread_spot}->{delta}->{0.9}->{day}->{2} = 0.006;
    $fx_per_symbol_config->{spread_vol}->{delta}->{0.9}->{day}->{2}  = 0.053;
    $fx_per_symbol_config                                            = JSON::MaybeXS::encode_json($fx_per_symbol_config);
    ...
    is sprintf("%.2f", $c->delta),                                             '0.93',    'delta is approximately 0.9';
    is business_days_between($c->date_start, $c->date_expiry, $c->underlying), 2,         '2 days expiry contract';
    is sprintf("%.5f", $c->bid_probability->amount),                           '6.30182', 'correct bid probability';
    is sprintf("%.5f", $c->ask_probability->amount),                           '6.38426', 'correct ask probability';
    ok $c->spread->amount != $c->spread_calculator->get_interpolated_spread({
            delta => abs($c->delta),
            vega  => $c->vega,
            t     => $c->timeinyears->amount,
            tenor => $c->get_tenor
        }
        ),
        'Not using interpolated spread for edge case';

    $fx_per_symbol_config                                            = JSON::MaybeXS::decode_json($fx_per_symbol_config);
    $fx_per_symbol_config->{spread_spot}->{delta}->{0.9}->{day}->{2} = 0;
    $fx_per_symbol_config->{spread_vol}->{delta}->{0.9}->{day}->{2}  = 0;
    $fx_per_symbol_config                                            = JSON::MaybeXS::encode_json($fx_per_symbol_config);
