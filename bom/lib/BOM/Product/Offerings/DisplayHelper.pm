...
my $offerings = BOM::Product::Offerings::DisplayHelper->new(offerings => LandingCompany::Registry->get_default_company()->basic_offerings());
    ...
    events => sub {
        ...
        if (my $cached = $self->holidays_cache->{$exchange->symbol}) {
            ...
            foreach my $day (0 .. $how_long) {
                ...
                if ($early_closes) {
                    my $is_partial_trading = defined $change_rules && ($change_rules->{daily_close}->{to} ne $early_closes->time . 'm');
