...
try {
    ...
    $log->infof('No CSV data for affiliate accumulator report for %s', $processing_date->date_yyyymmdd) unless @csv;
    ...
    $log->debugf('Sending email for affiliate accumulator report');
    ...
    try {
        unless ($zip->numberOfMembers) {
            $statsd->event('Failed to generate MyAffiliates accumulator report', 'MyAffiliates accumulator report generated an empty zip archive');
        ...
        unless ($zip->writeToFileNamed($output_zip_path) == AZ_OK) {
            $statsd->event('Failed to generate MyAffiliates accumulator report', "MyAffiliates accumulator report failed to generate zip archive");
            $log->warn('Failed to generate MyAffiliates accumulator report: MyAffiliates accumulator report failed to generate zip archive');
            exit 1;
        }
    } catch ($error) {
        $statsd->event('Failed to generate MyAffiliates accumulator report',
            "MyAffiliates accumulator report failed to generate zip archive with $error");
        $log->warnf("Failed to generate MyAffiliates accumulator report: MyAffiliates accumulator report failed to generate zip archive with %s",
            $error);
        exit 1;
    }
    ...
    $reporter->send_report(
        subject => 'CRON generate_affiliate_accumulator_commission_daily (' . $brand_object->name . ') for date ' . $processing_date->date_yyyymmdd,
        message => ["Find links to download CSV that was generated:\n" . $download_url],
    );
} catch ($error) {
    DataDog::DogStatsd->new->event('Affiliate Accumulator Report Failed', "AccumulatorReporter failed to generate csv files due: $error");
    $log->warnf("AccumulatorReporter failed to generate csv files due to: %s", $error);
    exit 1;
}
