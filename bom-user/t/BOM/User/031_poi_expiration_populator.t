...
subtest 'Best Date' => sub {
    ...
    for my $test ($tests->@*) {
        cmp_deeply BOM::User::Script::POIExpirationPopulator::get_best_date($test->{hash1}, $test->{hash2}), $test->{expected},
...
subtest 'get massive arrayref' => sub {
    ...
    for my $test ($tests->@*) {
        cmp_bag BOM::User::Script::POIExpirationPopulator::get_massive_arrayref($test->{hash}), $test->{expected}, 'Expected arrayref returned';
    }
};
...
subtest 'POI Expiration Populator' => sub {
    clear_poi_expiration();
    ...
    BOM::User::Script::POIExpirationPopulator::run({limit => 1});

    cmp_bag get_poi_expiration(), [$u1->id, $u2->id, $u4->id], 'Expected POI Expiration Dates';
    ...
    cmp_bag get_poi_expiration(), [$u1->id, $u4->id], 'Expected POI Expiration Dates';
        ...
        map { $_->{binary_user_id} } $user_db->run(
            fixup => sub {
                ...
                $_->selectall_arrayref(
                    'SELECT * FROM users.get_expired_poi(?, ?)',
                    {Slice => {}},
...
sub clear_poi_expiration {
    my $user_db = BOM::Database::UserDB::rose_db()->dbic;
    $user_db->run(
        fixup => sub {
            $_->do('DELETE FROM users.poi_expiration');
        });
}
