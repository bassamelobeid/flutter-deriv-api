...
sub _collect_rates_ages {
    ...
    foreach my $implied_symbol_to_update (@implied_symbols_to_update) {
        my $rates_in_used = Quant::Framework::ImpliedRate->new(symbol => $implied_symbol_to_update);
    ...
    foreach my $currency_symbol_to_update (@currencies_to_update) {
        ...
        my $currency_in_used = Quant::Framework::InterestRate->new(symbol => $currency_symbol_to_update);
    ...
    foreach my $smart_fx (@smart_fx) {
        my $smart_fx_in_used = Quant::Framework::InterestRate->new(symbol => $smart_fx);
