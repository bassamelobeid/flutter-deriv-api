...
my $SKIP_HOSTS = "$ENV{HOME}/.pnm_skip_hosts.conf"; # this file will have 'chronicle|chronicle01|chronicle02|crypto01|crypto03|mf04|mlt04|mx04|vr03|feed10|feed06|mlt06|mx06|dw02|dw04|dw05|
...
sub store_all_stats {
  ...
  my $centraldb = DBI->connect($dsn, '', '', {AutoCommit=>0, RaiseError=>1, PrintError=>0});
  ...
  $SQL = sprintf 'INSERT INTO %s(sampletime, host, %s) VALUES (?,?,%s)',
    $PSS_TABLE,
    (join ',' => sort @pss_fields),
      ('?,'x @pss_fields);
  $SQL =~ s/,\)/)/;
...
sub store_remote_stats {
  ...
  my $remotedb;
  eval {
    $remotedb = DBI->connect($dsn, '', '', {AutoCommit => 0, RaiseError=>0, PrintError=>1});
  };
  if (! defined $remotedb) {
...
sub load_hosts {
  ...
  for my $host (keys %hostinfo) {
    if (
        (length $arg{hosts} and $host !~ /$arg{hosts}/)
        or
        ($arg{skiphosts} and $host =~ /$arg{skiphosts}/)
       ) {
...
SELECT time, host, username, sumcalls, SUBSTR( TRIM(REGEXP_REPLACE(query, '[\n ]+', ' ', 'g')), 0, ?) AS query
 ...
 ORDER BY time DESC LIMIT ?
  ...
  my %longest = (host => 0, calls => 0, time => 0);
  for my $row (@$info) {
...
";
  ...
  for my $row (@$info) {
    ...
    $msg .= Text::Wrap::wrap(' ' x2, ' ' x 4, $row->{query});
    $msg .= "\n\n";
  }
...
EOT
  ...
  my $statsdb = DBI->connect($dsn, '', '', {AutoCommit => 0, RaiseError=>1, PrintError=>0});
