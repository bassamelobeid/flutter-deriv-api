...
while (1) {
    ...
    while (my $key = $redis->brpop("pricer_jobs", 0)) {
        ...
        DataDog::DogStatsd::Helper::stats_timing("pricer_daemon.$price_daemon_cmd.time", $response->{tv}*1000, {tags => ['tag:' . $internal_ip]});
        $response->{price_daemon_cmd} = $price_daemon_cmd;

        warn "Pricing time too long: " . ($response->{tv}*1000) . ' ' . Data::Dumper::Dumper($params) if $response->{rpc_time}>1;
