...
my $put_array = [grep { $_->{contract_type} eq 'PUTE' and $_->{trading_period}{duration} eq '2h' } @{$contracts_for->{contracts_for}{available}}];
unless (scalar @$put_array) {
    # fallback if there is no 2h contracts
    $put_array = [grep { $_->{contract_type} eq 'PUTE' and $_->{trading_period}{duration} eq '0d' } @{$contracts_for->{contracts_for}{available}}];
}
...
SKIP: {
    ...
    if ($put->{trading_period}{date_expiry}{epoch} - Date::Utility->new->epoch <= 900) {
        BAIL_OUT("Too close to the trading window border. Trading is not offered for this duration.". $put->{trading_period}{date_expiry}{epoch} . " now" .  Date::Utility->new->epoch);
        done_testing();
        exit;
    }
