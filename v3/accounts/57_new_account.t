...
subtest 'email and password likeness' => sub {
    ...
    $res                                = $t->await::new_account(\%create_vr_clone);
...
subtest 'create Virtual account' => sub {
    ...
    $res = $t->await::new_account($create_vr);
    is($res->{msg_type}, 'new_account');
    ok($res->{new_account});
    test_schema('new_account', $res);

    like($res->{new_account}->{client_loginid}, qr/^VRTC/, 'got VRTC client');
    is($res->{new_account}->{currency}, 'USD', 'got currency');
    cmp_ok($res->{new_account}->{balance}, '==', '10000', 'got balance');
};
