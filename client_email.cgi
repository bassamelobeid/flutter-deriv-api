...
if (not $input{update}) {
    ...
    code_exit_BO();
} elsif ($input{update} eq 'email') {
    unless ($input{transtype}) {
        print "Please select transaction type";
        code_exit_BO();
    }
    if (my $error = BOM::DualControl->new({staff => $clerk, transactiontype => $input{transtype}})->validate_client_control_code($input{DCcode}, $new_email)) {
