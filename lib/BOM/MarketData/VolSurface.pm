...
sub _build__ON_day {
    ...
    return $self->_exchange->calendar_days_to_trade_date_after(
        $self->effective_date );
}
...
around BUILDARGS => sub {
    ...
    if ( $args{surface} or $args{recorded_date} ) {
        ...
        if ( not $args{surface} or not $args{recorded_date} ) {
            croak(
'Must pass both "surface" and "recorded_date" if passing either.'
            );
        }
        ...
        foreach my $maturity ( keys %{ $args{surface} } ) {
            ...
            if ( _is_tenor($maturity) ) {
                $effective_date ||=
                  BOM::MarketData::VolSurface::Utils->new->effective_date_for(
                    $args{recorded_date} );
    ...
    if ( ref $args{original_term} ) {
          ...
          )
        {
            ...
            die(
'Given original_term attr must have both smile and vol_spread keys present.'
            );
        }
        foreach my $which (qw( smile vol_spread )) {
            if ( exists $args{original_term}->{$which} ) {
                $args{original_term}->{$which} = [
                    sort { $a <=> $b }
                      map { _is_tenor($_) ? $day_for_tenor{$_} : $_ }
                      @{ $args{original_term}->{$which} }
                ];
            }
        }
        ...
        foreach my $day ( keys %{$surface} ) {
            ...
            if ( $day < 30 and grep { $_ == $day } @market_points ) {
                ...
                if ( exists $surface->{$day}->{atm_spread}
                    and $surface->{$day}->{atm_spread} <
                    $self->extra_sd_vol_spread )
                {

                    $surface->{$day}->{atm_spread} +=
                      $self->extra_sd_vol_spread;
...
sub _compute_and_set_smile_spread {
    ...
    my @points =
      $self->_get_points_to_interpolate( $day,
        $self->original_term_for_spread );
    my $method =
    ...
    foreach my $spread_point ( @{ $self->spread_points } ) {
        ...
        if ( exists $surface->{$first_point}->{atm_spread} ) {
            ...
            $interpolation_second_point =
              $surface->{$second_point}->{atm_spread};
        }
        else {
...
sub _extrapolate_smile_spread {
    ...
    my $extrapolation_direction =
      $self->_get_extrapolate_direction( $seek, $points );
    my $extrapolation_method =
      '_extrapolate_smile_spread_' . $extrapolation_direction;
    ...
    if ( exists $surface->{$day} and exists $surface->{$day}->{atm_spread} ) {
          ...
          )
        {
            ...
            if ( $atm_spread_point eq 'atm_spread' ) {
                ...
                $atm_spread =
                  $smile_spread->{'vol_spread'}->{$atm_spread_point};
            }
...
sub _extrapolate_atm_spread {
    ...
    my $extrapolation_direction =
      $self->_get_extrapolate_direction( $seek, $points );
    my $extrapolation_method =
      '_extrapolate_atm_spread_' . $extrapolation_direction;
...
sub _market_maturities_interpolation_function {
    ...
    return sub {
        my ( $vol1, $vol2 ) = @_;
        return
          sqrt( ( $tau1 / $tau2 ) * ( $T2 / $T ) * $vol2**2 +
              ( ( $tau2 - $tau1 ) / $tau2 ) * ( $T1 / $T ) * $vol1**2 );
    };
}
...
sub get_volatility {
    ...
    if ( not $vol ) {
        my $emsg =
            "Could not interpolate volatility for "
          . $self->symbol
          . " for [$days] days and [$sought_point] delta.";
        warn($emsg)
          ;    # I think we are catching the error below for friendly output
        die "PricingError";
    }
    elsif ( $vol < 0 ) {
        my $emsg =
...
sub _validate_sought_values {
    ...
    if ( notall { defined $_ } ( $sought_point, $day ) ) {
        ...
        $day          ||= '';
        croak(
"Days[$day] or sought_point[$sought_point] is undefined for underlying["
              . $self->symbol
              . "]" );
    }
    ...
    if ( $day <= 0 or $sought_point < 0 ) {
        croak(
"get_volatility requires positive numeric days[$day] and sought_point[$sought_point]"
        );
    }
...
sub _compute_and_set_smile {
    ...
    my @points =
      $self->_get_points_to_interpolate( $day, $self->original_term_for_smile );
    my $method =
      ( $self->_is_between( $day, \@points ) )
      ? '_interpolate_smile'
      : '_extrapolate_smile';
...
sub _interpolate_smile {
    ...
    my $second_point = $points->[1];
    my $interpolate_func =
      $self->_market_maturities_interpolation_function( $seek, $first_point,
        $second_point );
    my $interpolated_smile;
...
# This can potentially cause problems if the effective date (surface recorded date) is too old
    my $days = $self->_vol_utils->effective_date_for($expiry_date)
      ->days_between( $self->effective_date );
...
sub get_rr_bf_for_smile {
    ...
    my $result = {
        ...
        BF_25 => ( $market_smile->{25} + $market_smile->{75} ) / 2 -
          $market_smile->{50},
    };
    if ( exists $market_smile->{10} ) {
        $result->{RR_10} = $market_smile->{10} - $market_smile->{90};
        $result->{BF_10} = ( $market_smile->{10} + $market_smile->{90} ) / 2 -
          $market_smile->{50};
    }
    return $result;
}
