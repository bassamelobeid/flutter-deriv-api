...
has directory_to_save => (
    is      => 'ro',
    default => sub {
        return BOM::Platform::Runtime->instance->app_config->system->directory
          ->feed . '/market';
    }
);
...
sub run {
    ...
    if ( $#files == -1 ) {
        push @{ $report->{error} },
          'OHLC AutoUpdater is terminating prematurely. File does not exist';
        return;
    }

    if ( $#files > 1000 ) {
        push @{ $report->{error} },
'OHLC AutoUpdater is terminating prematurely. Number of files in Bloomberg seems too big: ['
          . $#files . ']';
        return;
    }
    ...
    foreach my $file (@files) {
        ...
        my %bloomberg_to_binary =
          Bloomberg::UnderlyingConfig::bloomberg_to_binary;
        ...
        foreach my $data (@$csv) {
            ...
            unless ($bom_underlying_symbol) {
                push @{ $report->{error} },
                  "Unregconized bloomberg symbol[$bb_symbol]";
                next;
            }

            my $underlying =
              BOM::Market::Underlying->new($bom_underlying_symbol);
            my $now = Date::Utility->new;

            next if ( $underlying->has_holiday_on($now) );

            if ( my $validation_error =
                $self->_passes_sanity_check( $data, $bom_underlying_symbol ) )
            {
                ...
                my $market_db_file_path =
                  $self->directory_to_save . '/' . $symbol . '.db';
                my $line_to_append = "$date $open $high $low $close\n";
                if ( -e $market_db_file_path ) {
                    my $last_line = `tail -1 $market_db_file_path`;
                    if ( $last_line =~
/(\d\d?\-\w{3}\-\d{2})\s([\d\.]+)\s([\d\.]+)\s([\d\.]+)\s([\d\.]+)/
                      )
                    {
                        if ( $1 ne $date ) {
                            append_file( $market_db_file_path,
                                $line_to_append );
...
sub _passes_sanity_check {
    ...
    {
        return
"OHLC data error $symbol/$today: system retreived these from Bloomberg: open[$open] high[$high] low[$low] close[$close]";
    }
    elsif ( $open == $high and $high == $low and $low == $close ) {
        return
"OHLC date error $symbol/$today: open, high, low and close has the number";
    }
    elsif ( $open > $high ) {
        return
"OHLC data error $symbol/$today: open[$open] price bigger than high[$high] price";
    }
    elsif ( $open < $low ) {
        return
"OHLC data error $symbol/$today: open[$open] price less than low[$low] price";
    }
    elsif ( $close > $high ) {
        return
"OHLC data error $symbol/$today: close[$close] price bigger than high[$high] price";
    }
    elsif ( $close < $low ) {
        return
"OHLC data error $symbol/$today: close[$close] price less than low[$low] price";
    }
    elsif ( $high < $low ) {
        return
"OHLC data error $symbol/$today: high[$high] price less than low[$low] price";
    }
    elsif ( $high > $open * ( 1 + $suspicious_move ) ) {
        return
"OHLC suspicious data $symbol/$today: Suspicious : high ($high) > open ($open) + \%$p_suspicious_move";
    }
    elsif ( $close > $open * ( 1 + $suspicious_move ) ) {
        return
"OHLC suspicious data $symbol/$today: Suspicious : close ($close) > open ($open) + \%$p_suspicious_move";
    }
    elsif ( $low < $open * ( 1 - $suspicious_move ) ) {
        return
"OHLC suspicious data $symbol/$today: Suspicious : low ($low) < open ($open) - \%$p_suspicious_move";
    }
    elsif ( $close < $open * ( 1 - $suspicious_move ) ) {
        return
"OHLC suspicious data $symbol/$today: Suspicious : close ($close) < open ($open) - \%$p_suspicious_move";
...
sub verify_ohlc_update {
    ...
    foreach my $underlying_symbol (@underlying_symbols) {
        ...
        my $db_file =
          $self->directory_to_save . '/' . $underlying_symbol . '.db';

        if ( not $db_file ) {
            warn 'Could not find file ['
              . $self->directory_to_save . '/'
              . $underlying_symbol . '.db';
            next;
        }

        next
          if ( -M $db_file and -M $db_file >= 20 )
          ; # do only those that were modified in last 20 days (others are junk/tests)
...
sub _check_file {
    ...
    my (
        $prevwhen, $prevdate,  $prevopen, $prevhigh,
        $prevlow,  $prevclose, $date
    );

    foreach my $dbline (@filelines) {
        if ( $dbline =~
/^(\d\d?\-\w\w\w\-\d\d)\s+(\d*\.?\d*)\s+(\d*\.?\d*)\s+(\d*\.?\d*)\s+(\d*\.?\d*)/
          )
        {
            $date = $1;
            my $open  = $2;
            ...
            if ( $now->days_between($when) <=
                10 )    #don't bug cron with old suspicions
            {
                if ( $high > $open * ( 1 + $suspicious_move ) ) {
                    warn(
"--Warning: Suspicious : $underlying_symbol $date high ($high) > open ($open) + $p_suspicious_move\%"
                    );
                }
                elsif ( $close > $open * ( 1 + $suspicious_move ) ) {
                    warn(
"--Warning: Suspicious : $underlying_symbol $date close ($close) > open ($open) + $p_suspicious_move\%"
                    );
                }
                elsif ( $low < $open * ( 1 - $suspicious_move ) ) {
                    warn(
"--Warning: Suspicious : $underlying_symbol $date low ($low) < open ($open) - $p_suspicious_move\%"
                    );
                }
                elsif ( $close < $open * ( 1 - $suspicious_move ) ) {
                    warn(
...
"--Warning: Suspicious : $underlying_symbol $date high ($high) < previousclose ($prevclose) - $p_suspicious_move\%"
                ...
                if ( $high < $low ) {
                    warn(
"--ERROR : $underlying_symbol $date high ($high) < low ($low) !!"
                    );
                }
                elsif ( $close < $low ) {
                    warn(
"--ERROR : $underlying_symbol $date close ($close) < low ($low) !!"
                    );
                }
                elsif ( $close > $high ) {
                    warn(
"--ERROR : $underlying_symbol $date close ($close) > high ($high) !!"
                    );
                }
                ...
                if ( $prevwhen and $when->is_same_as($prevwhen) ) {
                    ...
                    if (
                        my $trading_days_between =
                        $underlying->exchange->trading_days_between(
                            $prevwhen, $when
                        )
                      )
                    {
...
"--Warning: $underlying_symbol MISSING DATES between $prevdate and $date (days between is: $days_between)."
            ...
            ( $prevwhen, $prevdate, $prevopen, $prevhigh, $prevlow, $prevclose )
              = ( $when, $date, $open, $high, $low, $close );
        }
        else {
    ...
    if (   $now->is_a_weekend
        or $now->day_of_week == 1
        and $date ne $now->date_ddmmmyy
        and $date ne $yesterday->date_ddmmmyy )
    {
        # Make sure we traded yesterday
        if ( $underlying->trades_on($yesterday) ) {
            warn(   "--$underlying_symbol ERROR can't find yesterday's data ("
                  . $yesterday->date_ddmmmyy
                  . ")" );
        }
    }
