...
use constant ALLOWABLE_SERVICES => [qw(p2p cashier_withdraw trading transfer_to_pa)];
...
# withdrawal limits
    ...
    if (defined $args->{services_allowed}) {
        die "ServicesAllowedInvalidType\n" unless ref $args->{services_allowed} eq 'ARRAY';
        for my $service ($args->{services_allowed}->@*) {
            die "ServicesAllowedInvalidName\n" unless any { $_ eq ($service // '') } ALLOWABLE_SERVICES->@*;
        }
        if (scalar $args->{services_allowed}->@*) {
            die "ServicesAllowedCommentsMissing\n" unless $args->{services_allowed_comments};
        } else {
            $args->{services_allowed_comments} = '';
        }
        $args->{services_allowed} = [uniq $args->{services_allowed}->@*];
    } else {
        delete $args->{services_allowed};
        delete $args->{services_allowed_comments};
    }
...
sub cashier_withdrawable_balance {
    ...
    for my $sibling ($self->sibling_payment_agents) {
        ...
        my $pa_commission  = sum map { $_->{deposits} } grep    { $payment_types{$_->{payment_type}} eq 'commission' } @payment_totals;
    ...
    my $limit = $self->service_is_allowed('cashier_withdraw') ? $self->client->account->balance : ($commission // 0) - ($payout // 0);
