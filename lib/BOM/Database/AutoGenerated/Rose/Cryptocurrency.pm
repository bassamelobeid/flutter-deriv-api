...
__PACKAGE__->meta->setup(
    ...
    columns => [
        ...
        transaction_type => {
            type     => 'enum',
            check_in => ['deposit', 'withdrawal'],
            db_type  => 'payment.payment_txn_type',
            not_null => 1
        },
        status => {
            type     => 'enum',
            check_in => [
                'NEW',        'PENDING', 'CONFIRMED', 'LOCKED',   'VERIFIED',                  'REJECTED',
                'PROCESSING', 'SENT',    'ERROR',     'RESOLVED', 'PERFORMING_BLOCKCHAIN_TXN', 'MIGRATED'
            ],
            db_type  => 'payment.ctc_status',
            not_null => 1
        },
