...
sub sell_by_shortcode {
    ...
    my $tmp_table_values = join
        ",\n    ",
        map {
            '(' . $_ . '::INT, $' . ($_ + 10) . '::VARCHAR(12))'; }
        1 .. scalar @{$self->account_data};

    my $stmt = $self->db->dbh->prepare('
WITH
acc( seq, loginid) AS (VALUES ' .$tmp_table_values . ')
SELECT acc.loginid, (v_fmb).*, (v_trans).*
  FROM acc
 CROSS JOIN LATERAL
 ...
 ORDER BY acc.seq');
    ...
    my @chld_col  = BOM::Database::AutoGenerated::Rose::FinancialMarketBet->meta->{relationships}->{$self->bet_data->{bet_class}}->class->meta->columns;
    my @qv_col    = BOM::Database::AutoGenerated::Rose::QuantsBetVariable->meta->columns;
    my $qv        = $self->quants_bet_variables;
    my $transdata = $self->transaction_data || {};
    my $chld_data = +{
    ...
    $stmt->execute(
        ...
        keys %$chld_data ? JSON::XS::encode_json($chld_data) : undef,                                                  # -- 6
    ...
    my $result;
    while (my $r = $stmt->fetchrow_arrayref) {
        my @row = @$r;
