...
sub sell_bet {
    ...
    use Data::Dumper;
    warn "Inside sell bet 0 ###" . Dumper($self->bet);
    warn "Inside sell bet 1 ###" . Dumper($bet);

    if ($self->bet) {
        $bet->{$_} //= $self->bet->$_ for (qw/id sell_price sell_time/);
    }

    warn "Inside sell bet####### " . Dumper($bet);
...
acc(account_id,currency_code)  AS (SELECT id, currency_code
                      ...
                      WHERE client_loginid=$1
                        AND currency_code=$2
                        FOR UPDATE),
bets(id, sell_price, sell_time, chld, transaction_time, staff_loginid, remark, source, quantity, qv) AS (VALUES
    ' . join(
        ",\n    ",
        map {
                  '($'
