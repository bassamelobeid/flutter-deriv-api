...
"SELECT loginid, first_name, last_name, date_of_birth, email from betonmarkets.client where upper(first_name)=? and upper(last_name)=? and date_of_birth=? and broker_code=?";
    ...
    if (@dupe_record) {
        get_logger()
          ->warn(
"Possible duplicate registration attempted.  Attempt matches - loginid: $dupe_record[0], firstname: $dupe_record[1], lastname: $dupe_record[2], email: $dupe_record[4]"
          );
    }
...
sub get_account_statistics {
    ...
    try {
        ...
        get_logger()
          ->warn(
            "BOM::Database::DataMapper::Client - get_account_statistics [$_]");
    };
