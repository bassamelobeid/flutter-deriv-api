...
=head2 get_last_txn_date_of_client

    get last used txn date of client

=cut

sub get_last_txn_date_of_client {
    my $self       = shift;
    my $txn_record = BOM::Database::AutoGenerated::Rose::Transaction::Manager->get_transaction(
        require_objects => ['account'],

        select => ['MAX(t1.transaction_time) as transaction_time',],
        query  => [
            client_loginid => $self->client_loginid,
        ],
        group_by => ['t2.client_loginid',],
        db       => $self->db
    );

    if (scalar @{$txn_record} > 0) {
        return $txn_record->[0]->transaction_time;
    }
