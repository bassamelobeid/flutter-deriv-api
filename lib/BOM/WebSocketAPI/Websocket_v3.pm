...
sub entry_point {
    ...
    if ( not $c->stash->{redis} ) {
        ...
        $redis->on(
            message => sub {
                ...
                BOM::WebSocketAPI::v3::Wrapper::Accounts::send_realtime_balance(
                    $c, $msg )
                  if $channel =~ /^TXNUPDATE::balance_/;
                BOM::WebSocketAPI::v3::Wrapper::Streamer::process_realtime_events(
                    $c, $msg )
                  if $channel =~ /^FEED::/;
            }
        );
    ...
    $c->on(
        json => sub {
            ...
            if ( ref($p1) eq 'HASH' ) {
                ...
                timeout 15 => sub {
                    $data =
                      _sanity_failed( $c, $p1 ) || __handle( $c, $p1, $tag );
                };
                if ($@) {
                    $c->app->log->info(
                        "$$ timeout for " . JSON::to_json($p1) );
                }
                ...
                $data =
                  $c->new_error( 'error', 'BadRequest',
                    $c->l('The application sent an invalid request.') );
                $data->{echo_req} = {};
            }
            $data->{version} = 3;
            ...
            if ( $l > 328000 ) {
                $data =
                  $c->new_error( 'error', 'ResponseTooLarge',
                    $c->l('Response too large.') );
                $data->{echo_req} = $p1;
            }
            if ($send) {
                $c->send( { json => $data } );
    ...
    $c->on(
        finish => sub {
            my $c = shift;
            BOM::WebSocketAPI::v3::Wrapper::System::forget_all( $c,
                { forget_all => 1 } );
...
for my $order ( 0 .. @dispatch - 1 ) {
    ...
    my $in_validator = JSON::Schema->new(
        JSON::from_json( File::Slurp::read_file("$f/send.json") ),
        format => \%JSON::Schema::FORMATS );
    my $out_validator = JSON::Schema->new(
        JSON::from_json( File::Slurp::read_file("$f/receive.json") ),
        format => \%JSON::Schema::FORMATS );
...
sub __handle {
    ...
    for my $descriptor (@handler_descriptors) {
        ...
        my $input_validation_result =
          $descriptor->{in_validator}->validate($p1);
        if ( not $input_validation_result ) {
            ...
            my $message = $c->l('Input validation failed: ')
              . join( ', ', ( keys %$details, @general ) );
            return $c->new_error( 'error', 'InputValidationFailed', $message,
                $details );
        }

        stats_inc( 'websocket_api_v3.call.' . $descriptor->{category},
            { tags => [$tag] } );
        stats_inc( 'websocket_api_v3.call.all',
            { tags => [ $tag, $descriptor->{category} ] } );
            ...
            and my $loginid = $c->stash('loginid') )
        {
            ...
            return $c->new_error( 'error', 'InvalidClient',
                $c->l('Invalid client account.') )
              unless $client;
            return $c->new_error( 'error', 'DisabledClient',
                $c->l('This account is unavailable.') )
                ...
                and Date::Utility->new->is_before( Date::Utility->new($lim) ) )
            {
                return $c->new_error(
                    ...
                    'ClientSelfExclusion',
                    $c->l(
                        'Sorry, you have excluded yourself until [_1].', $lim
                    )
                );
            }
        }
        ...
        if ( $descriptor->{require_auth} and not $c->stash('client') ) {
            return $c->new_error( $descriptor->{category},
                'AuthorizationRequired', $c->l('Please log in.') );
        }

        stats_inc( 'websocket_api_v3.authenticated_call.all',
            { tags => [ $tag, $descriptor->{category}, $c->stash('loginid') ] }
        );

        ## sell expired
        if ( grep { $_ eq $descriptor->{category} }
            ( 'portfolio', 'statement', 'profit_table' ) )
        {
            if ( BOM::Platform::Runtime->instance->app_config->quants->features
        ...
        if ($result) {
            my $output_validation_result =
              $descriptor->{out_validator}->validate($result);
            if ( not $output_validation_result ) {
                my $error = join( " - ", $output_validation_result->errors );
                $log->warn( "Invalid output parameter for [ "
                      . JSON::to_json($result)
                      . " error: $error ]" );
                return $c->new_error( 'OutputValidationFailed',
                    $c->l("Output validation failed: ") . $error );
            }
        }
        $result->{debug} = [
            Time::HiRes::tv_interval($t0),
            ( $c->stash('client') ? $c->stash('client')->loginid : '' )
          ]
          if ref $result;
        return $result;
    }
    ...
    return $c->new_error( 'error', 'UnrecognisedRequest',
        $c->l('Unrecognised request.') );
}
...
sub _failed_key_value {
    ...
    %pwd_field =
      map { $_ => 1 }
      qw( client_password old_password new_password unlock_password lock_password )
      if ( not %pwd_field );
...
sub rpc {
    ...
    $client->call(
        ...
        sub {
                ...
                $self->send(
                    {
                        json => $self->new_error(
                            'error', 'CallError',
                            $self->l( 'Call error.' . $res->error_message )
                        )
                    }
                );
                return;
            ...
            $data->{echo_req} = $args;
            $data->{version}  = 3;
            $data->{req_id}   = $args->{req_id}
              if ( $args and exists $args->{req_id} );

            my $l = length JSON::to_json($data);
            if ( $l > 328000 ) {
                $data =
                  $self->new_error( 'error', 'ResponseTooLarge',
                    $self->l('Response too large.') );
                $data->{echo_req} = $args;
            }
            if ($send) {
                $self->send( { json => $data } );
...
sub _sanity_failed {
  ...
  OUTER:
    ...
    if (@failed) {
        ...
        return $c->new_error( 'sanity_check', 'SanityCheckFailed',
            $c->l("Parameters sanity check failed.") );
    }
    return;
}
