    ...
    foreach my $symbol (@symbols) {
        ...
        if ( $response and exists $response->{error} ) {
            return $c->new_error(
                'ticks',
                $response->{error}->{code},
                $response->{error}->{message_to_client} );
        } else {
            if ( exists $args->{subscribe} and $args->{subscribe} eq '0' ) {
                    ...
                    = _feed_channel( $c, 'subscribe', $symbol, 'tick' ) )
                {
                    return $c->new_error(
                        ...
                        'AlreadySubscribed',
                        $c->l(
                            'You are already subscribed to [_1]', $symbol
                        ) );
                }
            }
        }
        ...
        sub {
            ...
            if ( exists $args->{subscribe} ) {
                if ( $args->{subscribe} eq '1' ) {
                    if (not _feed_channel(
                            $c,                     'subscribe',
                            $args->{ticks_history}, $response->{publish} ) )
                    {
                        return $c->new_error(
                            'ticks_history',
                            'AlreadySubscribed',
...
sub proposal {
    ...
    if ( $response and exists $response->{error} ) {
        ...
        my $id;
        if ( $args->{subscribe} eq '1' ) {
            $id = _feed_channel( $c, 'subscribe', $symbol,
                'proposal:' . JSON::to_json($args) );
        }
        send_ask( $c, $id, $args );
    }
    return;
            ...
            $c->send(
                {   json => {
                        ...
                        tick => {
                            ...
                            symbol => $symbol,
                            epoch  => $m[1],
                            quote  => BOM::Market::Underlying->new($symbol)
                                ->pipsized_value( $m[2] ) } } } ) if $c->tx;
        } elsif ( $type =~ /^proposal:/ and $m[0] eq $symbol ) {
            send_ask( $c, $feed_channels_type->{$channel}->{uuid},
                $arguments )
                if $c->tx;
        } elsif ( $type =~ /^proposal_open_contract:/ and $m[0] eq $symbol ) {
            BOM::WebSocketAPI::v3::Wrapper::PortfolioManagement::send_proposal(
                $c, $feed_channels_type->{$channel}->{uuid}, $arguments )
                if $c->tx;
        } elsif ( $m[0] eq $symbol ) {
