...
sub __handle {
    ...
    my @dispatch = (
        ...
        [
            'payout_currencies',
            \&BOM::WebSocketAPI::v2::ContractDiscovery::payout_currencies, 0
        ],
        [
            'active_symbols', \&BOM::WebSocketAPI::v2::Symbols::active_symbols,
            0
        ],
        [
            'contracts_for',
            \&BOM::WebSocketAPI::v2::ContractDiscovery::contracts_for, 0
        ],
        [
            'trading_times',
            \&BOM::WebSocketAPI::v2::MarketDiscovery::trading_times, 0
        ],
        [
            'asset_index',
            \&BOM::WebSocketAPI::v2::MarketDiscovery::asset_index, 0
        ],
        [ 'buy',  \&BOM::WebSocketAPI::v2::PortfolioManagement::buy,  1 ],
        [ 'sell', \&BOM::WebSocketAPI::v2::PortfolioManagement::sell, 1 ],
        [
            'portfolio',
            \&BOM::WebSocketAPI::v2::PortfolioManagement::portfolio, 1
        ],
        [
            'proposal_open_contract',
            \&BOM::WebSocketAPI::v2::PortfolioManagement::proposal_open_contract,
            1
        ],
        ...
        [
            'change_password',
            \&BOM::WebSocketAPI::v2::Accounts::change_password, 1
        ],
    );
    ...
    foreach my $dispatch (@dispatch) {
        ...
        my $f  = '/home/git/regentmarkets/bom-websocket-api/config/v2/'
          . $dispatch->[0];
        ...
        stats_inc( 'websocket_api_v2.call.' . $dispatch->[0],
            { tags => [$tag] } );
        stats_inc( 'websocket_api_v2.call.all',
            { tags => [ $tag, $dispatch->[0] ] } );
        ...
        stats_inc(
            'websocket_api_v2.authenticated_call.all',
            {
                tags => [
                    $tag, $descriptor->{category},
                    $c->stash('client')->loginid
                ]
            }
        );

        ## sell expired
        if ( grep { $_ eq $dispatch->[0] }
        ...
        if ( $result and not $validator->validate($result) ) {
            ...
            warn "Invalid output parameter for [ "
              . JSON::to_json($result)
              . " error: $error ]";
            return {
                msg_type => 'error',
