...
suite buy_then_sell_contract => sub {
    ...
    fmap_void {
        ...
        $context->connection(token => $args{token})->subscribe(buy => $request)->take(3)->chain(
            async sub {
                my $context     = shift;
        ...
        )->take_until(sub { ref($_[0]) eq 'Binary::API::OpenContract' && $_[0]->is_sold })->expect_done(
            sub {
                my @all_resps   = @_;
                my $buy_resp    = shift;
                my @poc_resps   = @_;
                ...
                # check buy response
                ok($contract_id, "Contract id $contract_id '" . $buy_resp->longcode . "' was purchased at buy price " . $buy_resp->buy_price);

                # check following responses
                ok(ref($poc_resps[$_]) eq 'Binary::API::OpenContract' && $poc_resps[$_]->contract_id == $contract_id, "poc response $_ is valid")
                    for (1 .. $#poc_resps);
                ok($sell_resp->is_sold == 1, "Contract '" . $sell_resp->longcode . "' was sold at bid price: " . $sell_resp->bid_price);
                ...
                ok(List::Util::all(sub { $buy_resp->envelope->subscription->id eq $_->envelope->subscription->id; }, @subscriptions),
                    'Subscription ids are all the same');
            }
        )->timeout_ok(
            5,
            sub { shift->take_latest },
            'There should be no more poc response after selling the contract'
            )->release->completed

    }
    foreach => [@buy_requests],
        %args{qw(concurrent)};
};
...
suite poc_no_contract_id => sub {
    ...
    my $con = $suite->last_context->connection(token => $args{token});

    $con->subscribe(proposal_open_contract => 1)->buy($buy_reqs[0]->{buy}->%*)->take_until(
        sub {
