...
suite publish_gap => sub {
    ...
    my $context  = $suite->last_context->connection(exists $args{token} ? %args{token} : ());
    fmap0 {
        my ($method, $request) = $_->%*;
        ...
        $published_method = 'p2p'  if $method =~ /^p2p_/;
        $published_method = 'tick' if $method eq 'ticks';

        $context->subscribe($method, $request)->pause_publish($published_method)->skip_until_publish_paused($published_method)
            # Wait 1 second to make sure no previous publish will be received
            ->chain(
            sub {
                shift->completed->then(sub { $suite->tester->loop->delay_future(after => 2) });
            }
        )->timeout_ok(
            2,
            sub { shift->take_latest }
            )->resume_publish($published_method)->take_latest->helper::log_method($request)->completed
    }
    foreach        => [$requests->@*],
