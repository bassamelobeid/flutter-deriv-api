package BOM::Test::Data::Utility::SetupDatasetTestFixture;
...
sub _setup_rates {
    ...
    if ($underlying->rate_to_imply eq $quoted_currency->symbol) {
        $quoted_currency = BOM::MarketData::InterestRate->new(
    ...
    if ($underlying->rate_to_imply eq $asset->symbol) {
        $asset = BOM::MarketData::InterestRate->new(
            symbol        => $asset->symbol,
...
sub _reset_rates {
    ...
    if ($underlying->rate_to_imply eq $underlying->quoted_currency->symbol) {
        $current_quoted_currency_data = BOM::MarketData::InterestRate->new(
    ...
    if ($underlying->rate_to_imply eq $underlying->asset->symbol) {
        $current_asset_data = BOM::MarketData::InterestRate->new(
