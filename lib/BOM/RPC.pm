...
sub _auth {
    ...
    my $client =
      Client::Account->new( { loginid => $token_details->{loginid} } );
...
sub _validate_tnc {
    ...
    my $client = $params->{client}
      // die "client should be authenticated before calling this action";
    return $params if $client->is_virtual;

    my $current_tnc_version = BOM::Platform::Runtime->instance->app_config->cgi
        ...
        return BOM::RPC::v3::Utility::create_error(
            {
                ...
                message_to_client =>
                  localize('Terms and conditions approval is required.'),
            }
        );
    }
    return $params;
...
sub _compliance_checks {
    ...
    # we shouldn't get to this error, so we can die it directly
    my $client = $params->{client}
      // die "client should be authed before calling this action";
    ...
    if ( ( $client->aml_risk_classification // '' ) eq 'high'
        and not $client->financial_assessment() )
    {
        return BOM::RPC::v3::Utility::create_error(
            {
...
sub _check_tax_information {
    ...
    if ( $client->landing_company->short eq 'maltainvest'
        and not $client->get_status('crs_tin_information') )
    {
        return BOM::RPC::v3::Utility::create_error(
            {
...
sub register {
    ...
    return MojoX::JSON::RPC::Service->new->register(
        ...
        sub {
            ...
            my $args = {};
            $args->{country_code} = $params->{country}
              if exists $params->{country};
            $params->{token} = $params->{args}->{authorize}
              if !$params->{token} && $params->{args}->{authorize};
            my $token_details =
              BOM::RPC::v3::Utility::get_token_details( $params->{token} );

            if (    $token_details
                and exists $token_details->{loginid}
                and $token_details->{loginid} =~ /^(\D+)\d+$/ )
            {
                $args->{broker_code} = $1;
            }
            ...
            if ( exists $params->{server_name} ) {
                $params->{website_name} = BOM::RPC::v3::Utility::website_name(
            ...
            for my $action (@local_before_actions) {
                ...
                try {
                    ...
                    cluck(
"Error happened when call before_action $action at method $method: $_"
                    );
                    $result = BOM::RPC::v3::Utility::create_error(
                        {
                            code => 'Internal Error',
                            message_to_client =>
                              localize('Sorry, there is an internal error.'),
                        }
                    );
                };
            ...
            if ( $params->{valid_source} ) {
                ...
                $verify_app_res = BOM::RPC::v3::App::verify_app(
            ...
            my $result = try {
                ...
                warn "Exception when handling $method - $_ with parameters "
                  . encode_json \@args;
...
"Sorry, an error occurred while processing your account."
            ...
            if ( $verify_app_res && ref $result eq 'HASH' ) {
                $result->{stash} = { %{ $result->{stash} // {} },
                    %{ $verify_app_res->{stash} } };
            }
            return $result;
        }
    );
...
sub startup {
    ...
    $app->moniker('rpc');
    $app->plugin(
        'Config' => { file => $ENV{RPC_CONFIG} || '/etc/rmg/rpc.conf' } );
    ...
    my @services = (
        ...
        [
            ...
            [qw(auth validate_tnc compliance_checks check_tax_information)]
        ],
        [
        ...
        [
            ...
            \&BOM::RPC::v3::MarketDiscovery::get_corporate_actions
        ],
        ...
        [
            'get_account_status', \&BOM::RPC::v3::Accounts::get_account_status,
            [qw(auth)]
        ],
        [
            'change_password', \&BOM::RPC::v3::Accounts::change_password,
            [qw(auth)]
        ],
        [
            'cashier_password', \&BOM::RPC::v3::Accounts::cashier_password,
            [qw(auth)]
        ],
        ...
        [
            'get_self_exclusion', \&BOM::RPC::v3::Accounts::get_self_exclusion,
            [qw(auth)]
        ],
        [
            'set_self_exclusion', \&BOM::RPC::v3::Accounts::set_self_exclusion,
            [qw(auth)]
        ],
        ...
        [
            ...
            \&BOM::RPC::v3::Accounts::set_account_currency,
            [qw(auth)]
        ],
        [ 'tnc_approval', \&BOM::RPC::v3::Accounts::tnc_approval, [qw(auth)] ],
        [
            'set_financial_assessment',
            \&BOM::RPC::v3::Accounts::set_financial_assessment,
            [qw(auth)]
        ],
        [
            'get_financial_assessment',
            \&BOM::RPC::v3::Accounts::get_financial_assessment,
            [qw(auth)]
        ],
        [
        ...
        [
            'new_sub_account', \&BOM::RPC::v3::NewAccount::new_sub_account,
            [qw(auth)]
        ],
        [
            'jp_knowledge_test',
            \&BOM::RPC::v3::Japan::NewAccount::jp_knowledge_test,
            [qw(auth)]
        ],

        [
            'portfolio', \&BOM::RPC::v3::PortfolioManagement::portfolio,
            [qw(auth)]
        ],
        [
        ...
        [
            ...
            \&BOM::RPC::v3::PortfolioManagement::proposal_open_contract,
            [qw(auth)]
        ],
        ...
        [
            'mt5_login_list', \&BOM::RPC::v3::MT5::Account::mt5_login_list,
            [qw(auth)]
        ],
        [
            'mt5_new_account', \&BOM::RPC::v3::MT5::Account::mt5_new_account,
            [qw(auth)]
        ],
        [
            'mt5_get_settings', \&BOM::RPC::v3::MT5::Account::mt5_get_settings,
            [qw(auth)]
        ],
        [
            'mt5_set_settings', \&BOM::RPC::v3::MT5::Account::mt5_set_settings,
            [qw(auth)]
        ],
        [
            'mt5_password_check',
            \&BOM::RPC::v3::MT5::Account::mt5_password_check,
            [qw(auth)]
        ],
        [
            'mt5_password_change',
            \&BOM::RPC::v3::MT5::Account::mt5_password_change,
            [qw(auth)]
        ],
        [
        ...
        [
            'mt5_withdrawal', \&BOM::RPC::v3::MT5::Account::mt5_withdrawal,
            [qw(auth)]
        ],

        [
            'copytrading_statistics',
            \&BOM::RPC::v3::CopyTrading::Statistics::copytrading_statistics
        ],
        ...
        'json_rpc_dispatcher' => {
            ...
            exception_handler => sub {
                ...
                DataDog::DogStatsd::Helper::stats_inc(
                    'bom_rpc.v_3.call_failure.count',
                    { tags => ["rpc:$path"] }
                );
    ...
    $app->hook(
        before_dispatch => sub {
            ...
            $call = $c->req->url->path;
            $0    = "bom-rpc: "
              . $call;    ## no critic (RequireLocalizedPunctuationVars)
            $call =~ s/\///;
            $request_start = [Time::HiRes::gettimeofday];
            DataDog::DogStatsd::Helper::stats_inc( 'bom_rpc.v_3.call.count',
                { tags => ["rpc:$call"] } );
        }
    );
    ...
    $app->hook(
        after_dispatch => sub {
            ...
            DataDog::DogStatsd::Helper::stats_inc(
                'bom_rpc.v_3.call_success.count',
                { tags => ["rpc:$call"] } );
            ...
            DataDog::DogStatsd::Helper::stats_timing( 'bom_rpc.v_3.cpuusage',
                $cpu->usage(), { tags => ["rpc:$call"] } );

            push @recent,
              [
                $request_start,
                Time::HiRes::tv_interval( $request_end, $request_start )
              ];
            ...
            $usage = sprintf( '%.2f',
                100 * $usage /
                  Time::HiRes::tv_interval( $request_end, $recent[0]->[0] ) );

            $0 = "bom-rpc: (idle since $end #req=$request_counter us=$usage%)"
