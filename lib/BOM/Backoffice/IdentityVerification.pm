        ...
        map {
            ...
            (
                map {
                    my $document_type = $_;
                    my $display_name =
                        ($document_type eq 'national_id')
                        ? 'National ID Number'
                        : $country_config->{document_types}->{$_}->{display_name};
                    defined $display_name ? ($document_type => $display_name) : ();
                } keys $country_config->{document_types}->%*
            );
        } keys $config->%*
    };
