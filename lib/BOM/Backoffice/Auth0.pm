...
sub login {
    ...
    if ($user) {
        ...
        BOM::Platform::RedisReplicated::redis_write->set("BINARYBOLOGIN::" . $user->{nickname}, JSON::MaybeXS->new->utf8->encode($user));
...
sub from_cookie {
    ...
    if ($staff and $user = BOM::Platform::RedisReplicated::redis_read->get("BINARYBOLOGIN::" . $staff)) {
        return JSON::MaybeXS->new->utf8->decode($user);
    }
    return;
}
...
sub has_authorisation {
    ...
    my $user;
    if ($cache and $user = JSON::MaybeXS->new->utf8->decode($cache) and $user->{token} = $auth_token) {
