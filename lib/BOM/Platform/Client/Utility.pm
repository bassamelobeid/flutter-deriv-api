...
sub encrypt_secret_answer {
    ...
    return BOM::Utility::Crypt->new(keyname => 'client_secret_answer')->encrypt_payload(data => $secret_answer);
}

sub decrypt_secret_answer {
    my $secret_answer = shift;
    if ($secret_answer =~ /^\w+\*.*\./) {    # new AES format
        return BOM::Utility::Crypt->new(keyname => 'client_secret_answer')->decrypt_payload(value => $secret_answer);
