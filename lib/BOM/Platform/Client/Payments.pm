...
sub validate_payment {
    ...
    if ($action_type eq 'withdrawal') {
        ...
        if ($lc eq 'costarica') {
            my $cr_limit = $lc_limits->limit_for_days;
            if ($absamt >= $cr_limit) {
                die "Withdrawal amount [$currency $absamt] exceeds withdrawal limit [$currency $cr_limit].\n";
            }
...
sub deposit_virtual_funds {
    my $self     = shift;
    my $amount   = shift || BOM::Platform::Runtime->instance->app_config->payments->virtual->topup_amount;
...
sub validate_agent_payment {
    ...
    $pa_total_amount = amount_from_to_currency($pa_total_amount, $payment_agent->default_account->currency_code, 'USD');
