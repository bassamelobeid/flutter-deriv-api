...
sub new {                                    ## no critic RequireArgUnpack
    ...
    if ($self->{token}) {
        $self = eval { JSON::from_json(BOM::Plarform::RedisReplicated::redis_read()->get('VERIFICATION_TOKEN::' . $self->{token})) } || {};
    ...
    if (my $token = BOM::Plarform::RedisReplicated::redis_write()->get('VERIFICATION_TOKEN_INDEX::' . $key)) {
        BOM::Plarform::RedisReplicated::redis_write()->del('VERIFICATION_TOKEN::' . $token);
    }

    $self->{expires_in} ||= 3600;
    BOM::Plarform::RedisReplicated::redis_write()->set('VERIFICATION_TOKEN::' . $self->{token}, JSON::to_json($self));
    BOM::Plarform::RedisReplicated::redis_write()->expire('VERIFICATION_TOKEN::' . $self->{token}, $self->{expires_in});
    BOM::Plarform::RedisReplicated::redis_write()->set('VERIFICATION_TOKEN_INDEX::' . $key, $self->{token});
    BOM::Plarform::RedisReplicated::redis_write()->expire('VERIFICATION_TOKEN_INDEX::' . $key, $self->{expires_in});
    return bless $self, $package;
}
...
sub delete_token {    ## no critic
    ...
    BOM::Plarform::RedisReplicated::redis_write()->del('VERIFICATION_TOKEN::' . $self->{token});
    BOM::Plarform::RedisReplicated::redis_write()->del('VERIFICATION_TOKEN_INDEX::' . md5_hex($self->{created_for} . $self->{email}))
