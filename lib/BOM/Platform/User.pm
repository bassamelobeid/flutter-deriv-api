...
sub login {
    ...
    if (BOM::Platform::Runtime->instance->app_config->system->suspend->all_logins) {
        ...
        BOM::Plarform::AuditLog::log('system suspend all login', $self->email);
        ...
        BOM::Plarform::AuditLog::log('failed login > 5 times', $self->email);

    } elsif (not $is_social_login and not BOM::Plarform::Password::checkpw($args{password}, $self->password)) {
        ...
        BOM::Plarform::AuditLog::log('incorrect email or password', $self->email);
        ...
        BOM::Plarform::AuditLog::log('Account disabled', $self->email);
    } elsif (
        @self_excluded = grep {
        ...
        )
    {
        ...
        BOM::Plarform::AuditLog::log('Account self excluded', $self->email);
    }
    ...
    BOM::Plarform::AuditLog::log('successful login', $self->email);
