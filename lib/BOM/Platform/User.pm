...
use BOM::Database::AutoGenerated::Rose::Users::BinaryUser::Manager;
...
sub new {
    ...
    if ($args->{loginid}) {
        return try {
            my $self = BOM::Database::AutoGenerated::Rose::Users::BinaryUser::Manager->get_binary_user(
                with_objects => ['loginid'],
                query        => [loginid => $args->{loginid}],
                db           => BOM::Database::UserDB::rose_db(),
            )->[0];
            bless $self, $class;
        }
