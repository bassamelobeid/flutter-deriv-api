package BOM::Platform::Data::CouchDB::Connection;

=head1 NAME

BOM::Platform::Data::CouchDB::Connection

=head1 SYNOPSYS

    my $couch_connection = BOM::Platform::Data::CouchDB::Connection->new(
        host   => 'localhost',
        port   => 5432,
        protocol => 'http://',
        couch => 'testdb',
...
use BOM::Platform::Data::CouchDB::Exceptions;
...
sub document {
    ...
    try { $couch_doc->retrieve; }
        ...
        BOM::Platform::Data::CouchDB::RetrieveFailed->throw(
            db       => $self->db,
            uri      => $self->log_uri,
            document => $doc,
            error    => $_,
    ...
    if ($data) {
        if (exists $data->{_rev}) {
            if ($data->{_rev} ne $couch_doc->rev) {
                BOM::Platform::Data::CouchDB::RevisionNotMatched->throw(
                    uri      => $self->log_uri,
                    db       => $self->db,
                    document => $doc,
        ...
        try {
            ...
            BOM::Platform::Data::CouchDB::UpdateFailed->throw(
                uri      => $self->log_uri,
                db       => $self->db,
                document => $doc,
                error    => $_,
...
sub view {
    ...
    try { $result = $couch_design_doc->queryView($view, %$params); }
        ...
        BOM::Platform::Data::CouchDB::QueryFailed->throw(
            db    => $self->db,
            uri   => $self->log_uri,
            view  => $view,
            error => $_,
...
sub create_document {
    ...
    try { $couch_doc->create; }
        ...
        BOM::Platform::Data::CouchDB::Exception->throw(
            uri     => $self->log_uri,
            error   => $_,
...
sub _build__couchdb_client {
    ...
    unless ($client->testConnection) {
        BOM::Platform::Data::CouchDB::ConnectionFailed->throw(
            uri   => $self->log_uri,
