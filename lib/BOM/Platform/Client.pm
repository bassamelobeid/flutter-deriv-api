...
sub siblings {
    ...
    my $binary_user = BOM::Database::AutoGenerated::Rose::Users::BinaryUser->new(email => $email);
    my @siblings    = sort { (($a->is_virtual ? 'V' : 'R') . $a->loginid) cmp(($b->is_virtual ? 'V' : 'R') . $b->loginid) }
        map { BOM::Platform::Client->new({loginid => $_, db_operation => 'replica'}) } $binary_user->loginid;
    return @siblings;
}
