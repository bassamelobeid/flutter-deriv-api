...
# get my 'User' record out of the UserDB
sub binary_user {
    my $self = shift;
    return BOM::Database::AutoGenerated::Rose::Users::BinaryUser->new({email => $self->email});
}

# get my siblings, in loginid order but with reals up first.  Use the replica db for speed.
# Very unfortunate layout here by perltidy.
sub siblings {
    my $self = shift;
    return sort { ($a->is_virtual ? 'V' : 'R') . $a->loginid cmp($b->is_virtual ? 'V' : 'R') . $b->loginid }
        map { BOM::Platform::Client->new({loginid => $_, db_operation => 'replica'}) } $self->binary_user->loginid;
}

# generate the string needed by the loginid_list cookie (but remove the loginid_list cookie soon!)
sub loginid_list_cookie_val {
    my $self = shift;
    return join('+', map { $_->loginid . ':' . ($_->is_virtual ? 'V' : 'R') } $self->siblings);
}
