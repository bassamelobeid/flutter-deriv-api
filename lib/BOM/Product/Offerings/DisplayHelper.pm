...
my %known_decorations = (
    ...
    times => sub {
        ...
        if ( my $open =
            $trading_calendar->opening_on( $exchange, $self->date ) )
        {
            push @{ $times->{open} }, $open->$display_method;
            my @closes;
            push @closes,
              $trading_calendar->closing_on( $exchange, $self->date );
            $times->{settlement} =
              $trading_calendar->get_exchange_open_times( $exchange,
                $self->date, 'daily_settlement' )->$display_method;
            if ( my $breaks =
                $trading_calendar->trading_breaks( $exchange, $self->date ) )
            {
                for my $break (@$breaks) {
    ...
    events => sub {
        ...
        if ( my $cached = $self->holidays_cache->{ $exchange->symbol } ) {
            ...
            my $trading_day =
              $trading_calendar->trading_date_for( $exchange, $self->date );
         ...
         # Put a note here when you discover the exception.
                my ( $rule, $message );
                my $change_rules =
                  $trading_calendar->regularly_adjusts_trading_hours_on(
                    $exchange, $when );
                my $early_closes =
                  $trading_calendar->closes_early_on( $exchange, $when );
                if ($early_closes) {
...
# Only set the rule as Friday if it is early close due to Friday.
                    $rule = $change_rules->{daily_close}->{rule}
                      if ( defined $change_rules->{daily_close}->{rule}
                        and $early_closes->hour
                        . 'h' eq $change_rules->{daily_close}->{to} );
                    $message =
                      $self->c
                      ? $self->c->l(
                        'Closes early (at [_1])',
                        $trading_calendar->closing_on( $exchange, $when )
                          ->time_hhmm
                      )
                      : 'Closes early (at '
                      . $trading_calendar->closing_on( $exchange, $when )
                    ...
                    $rule = $change_rules->{daily_open}->{rule}
                      if ( defined $change_rules->{daily_open}->{rule}
                        and $trading_calendar->opens_late_on( $exchange, $when )
                        ->hour . 'h' eq $change_rules->{daily_open}->{to} );
                    $message =
                      $self->c
                      ? $self->c->l(
                        'Opens late (at [_1])',
                        $trading_calendar->opening_on( $exchange, $when )
                          ->time_hhmm
                      )
                      : 'Opens late (at '
                      . $trading_calendar->opening_on( $exchange, $when )
                    ...
                    my $holiday_desc = $trading_calendar->is_holiday_for(
                        $exchange->symbol, $when
                    )
                  )
                {
...
# I'd rather deal with that here than in TT.
                    ...
                    my $where =
                      first_index { $_->{descrip} eq $message } @events;
...
sub _build_tree {
      ...
      )
    {
        ...
        foreach my $submarket (
            sort { $a->display_order <=> $b->display_order }
            map  { Finance::Asset::SubMarket::Registry->instance->get($_) }
            $offerings_obj->query( { market => $market->name }, ['submarket'] )
          )
        {
            my $children       = [];
            my $submarket_info = {
              ...
              )
            {
                  ...
                  )
                {
                    ...
                    push @{ $underlying_info->{contract_categories} },
                      $category_info;
                }
    ...
    foreach my $level ( keys %decorations ) {
        foreach my $item ( @{ $self->get_items_on_level($level) } ) {
            foreach my $as ( keys %{ $decorations{$level} } ) {
                ...
                my $sub =
                  ( ref $func ? $func : $known_decorations{$func} ) || next;
                local $_ = $item->{obj};
