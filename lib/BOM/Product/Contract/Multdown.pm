...
Profit & Loss (PnL) calculation of Multiplier Down Option is as follow:

    $PnL = $stake * (-1 * (($current_spot - $basis_spot)/ $basis_spot - $commission) * $multplier);
...
The Stop Out of a Multiplier Up Option is always higher than it basis spot.
...
Pricing model for deal cancellation with take profit:
    ...
    $cancellation_price = $stake * $multipler * (_american_binary_knockout() + _double_knockout());

Pricing model for deal cancellation without take profit:

    $cancellation_price = $stake * $multiplier * _standard_barrier_option();
...
sub _standard_barrier_option {
    ...
    my $price = Math::Business::BlackScholesMerton::NonBinaries::standardbarrier($spot, $self->_stop_out_proxy, $exercise_price, $cash_rebate,
        $args->{t}, $args->{r}, $args->{q}, $args->{sigma}, $self->_type);
...
sub _american_binary_knockout {
    ...
    my $payout                 = $spot - $spot * (1 - $take_profit_percentage / $self->multiplier) * exp(-1 * $self->_barrier_continuity_adjustment);

    my $price = Math::Business::BlackScholesMerton::Binaries::americanknockout($spot, $self->_stop_out_proxy, $self->_take_profit_proxy, $payout,
...
sub _double_knockout {
    ...
    my $price =
        Math::Business::BlackScholesMerton::NonBinaries::doubleknockout($spot, $self->_stop_out_proxy, $self->_take_profit_proxy, $K, $args->{t},
...
sub _build__take_profit_proxy {
    ...
    return $self->_spot_proxy *
        (1 - $take_profit_percentage / $self->multiplier - $self->commission) *
        exp(-1 * $self->_barrier_continuity_adjustment);
}
...
sub _type {
    return 'p';    # to represent BUY/LONG
}
