package BOM::Product::Contract::Turbosput;
...
# barrier_category add Turbosput
...
sub _build_ask_probability {
    ...
    my $prob = Math::Util::CalculatedValue::Validatable->new({
        ...
        minimum     => 0,
        base_amount => $self->barrier->as_absolute - $self->theo_ask_probability,
    });
...
sub _build_bid_probability {
    ...
    my $prob = Math::Util::CalculatedValue::Validatable->new({
        ...
        set_by      => 'BOM::Product::Role::Turbos',
        minimum     => 0,
        base_amount => $self->barrier->as_absolute - $self->theo_bid_probability,
    });
...
sub calculate_payout {
    ...
    return financialrounding('price', $self->currency, $self->barrier->as_absolute - $self->current_spot);
}
