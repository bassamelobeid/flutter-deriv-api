...
sub available_contracts_for_symbol {
    ...
    for my $o (@offerings) {
        ...
        if ($predefined_contract) {
            ...
            my $current_spot =
                defined $underlying->spot_tick ? $underlying->spot_tick->quote : $underlying->tick_at(time, {allow_inconsistent => 1})->quote;

            if ($o->{barriers} == 1 or $bc eq 'euro_atm') {
                my @barriers = sort { abs($current_spot - $a) <=> abs($current_spot - $b) } @{$o->{available_barriers}};
                $o->{barrier} = $barriers[0];
            } elsif ($o->{barriers} == 2) {
                my @barriers2 = sort { abs($current_spot - $a->[0]) <=> abs($current_spot - $b->[0]) } @{$o->{available_barriers}};
...
sub _get_barrier {
    ...
    $duration = ($duration =~ /(\d+)/) ? $duration : Time::Duration::Concise->new(interval => $duration)->seconds;

    # latest available spot should be sufficient.
    $barrier_spot =
          ($barrier_spot)                ? $barrier_spot
        : defined $underlying->spot_tick ? $underlying->spot_tick
        :                                  $underlying->tick_at(time, {allow_inconsistent => 1});
...
sub _predefined_barriers_on_trading_period {
    ...
    my $number_of_barrier = $contract->{barriers} ? $contract->{barriers} : $contract->{barrier_category} eq 'euro_atm' ? 1 : 0;
