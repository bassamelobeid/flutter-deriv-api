...
sub _build_staking_limits {
    ...
    push @possible_payout_maxes, $bet_limits->{maximum_payout};
    push @possible_payout_maxes, $bet_limits->{maximum_payout_on_new_markets}
        if ($underlying->is_newly_added);
    push @possible_payout_maxes, $bet_limits->{maximum_payout_on_less_than_7day_indices_call_put}
    ...
    my $payout_min = ($self->underlying->market->name eq 'random') ? $bet_limits->{minimum_payout_for_random} : $bet_limits->{minimum_payout};
