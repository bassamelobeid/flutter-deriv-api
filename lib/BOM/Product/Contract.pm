...
sub _validate_payout {
    ...
    if ($bet_payout > $payout_max) {
        return {
            message => 'payout amount outside acceptable range '.
                "[given: ".$bet_payout."] ".
                "[min: ".$payout_min."] ".
                "[max: ".$payout_max."]",
    ...
    if ($bet_payout =~ /\.[0-9]{3,}/) {
        ...
        return {
            message => 'payout amount has too many decimal places '.
                "[permitted: 2] ".
                "[payout: ".$bet_payout."]",
...
sub _validate_stake {
    ...
    if ($contract_stake < $stake_minimum) {
        return {
            message => 'stake is not within limits '.
...
sub _validate_input_parameters {
    ...
    if ($epoch_expiry == $epoch_start) {
        return {
            message => 'Start and Expiry times are the same '.
                "[start: ".$epoch_start."] ".
                "[expiry: ".$epoch_expiry."]",
...
sub _validate_trading_times {
    ...
    if (not($calendar->trades_on($date_start) and $calendar->is_open_at($date_start))) {
        ...
        return {
            message => 'underlying is closed at start '.
                "[symbol: ".$underlying->symbol."] ".
    ...
    if ($self->is_intraday) {
        ...
        if ($underlying->market->equity and $trading_days <= 4 and $holiday_days >= 2) {
            ...
            my $times_link = request()->url_for('/resources/market_timesws', undef, {no_host => 1});
            return {
                message => 'Not enough trading days for calendar days '.
