...
sub decorate {
    ...
    foreach my $offering (@$offerings) {
        ...
        if ($offering->{contract_category} ne 'callputequal' or $offering->{contract_category} ne 'callput') {
        ...
        my $maximum_contract_duration =
            ($offering->{contract_category} eq 'callputequal' or $offering->{contract_category} eq 'callput' and $offering->{expiry_type} eq 'intraday')
            ? 21600
