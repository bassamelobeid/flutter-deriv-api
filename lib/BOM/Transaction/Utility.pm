...
sub _extract_accumulator_limit_orders {
    my $contract_params = shift;

    my %orders = ();

    if (ref $contract_params eq 'BOM::Database::AutoGenerated::Rose::FinancialMarketBet') {
        my $child = $contract_params->{accumulator};

        if ($child->take_profit_order_amount) {
            $orders{'take_profit'} = {
                date   => $child->take_profit_order_date,
                amount => $child->take_profit_order_amount,
            };
        }

    } elsif (ref $contract_params eq 'HASH') {
        if ($contract_params->{take_profit_order_amount}) {
            $orders{'take_profit'} = {
                date   => $contract_params->{take_profit_order_date},
                amount => $contract_params->{take_profit_order_amount},
            };
        }
