...
sub extract_limit_orders {
    ...
    return $is_multiplier_contract
        ...
        : _extract_take_profit($contract_parameters);
}

=head2 _extract_multiplier_limit_order

parse parameters like stop_out and take_profit from financial_market_bet for a multiplier contract

=cut

sub _extract_multiplier_limit_orders {
...
=head2 _extract_accumulator_limit_order

parse take_profit parameter from financial_market_bet for a accumulator contract

=cut

sub _extract_take_profit {
    my $contract_params = shift;

    my %orders = ();

    if (ref $contract_params eq 'BOM::Database::AutoGenerated::Rose::FinancialMarketBet') {
        my $child = $contract_params->{accumulator} // $contract_params->{turbos};

        if ($child->take_profit_order_date) {
            $orders{'take_profit'} = {
                order_date   => $child->take_profit_order_date->epoch,
                order_amount => $child->take_profit_order_amount,
            };
        }

    } elsif (ref $contract_params eq 'HASH') {
        if ($contract_params->{take_profit_order_date}) {
            $orders{'take_profit'} = {
                order_date   => $contract_params->{take_profit_order_date},
                order_amount => $contract_params->{take_profit_order_amount},
            };
        }
