...
sub _validate_offerings {
    ...
    my $method = $client->landing_company->short =~ /japan/ ? 'multi_barrier_offerings_for_country' : 'offerings_for_country';
...
sub _validate_sell_pricing_adjustment {
    ...
    if ($contract->is_binary) {
        $self->_validate_sell_pricing_adjustment_binary;
    } else {
        $self->_validate_sell_pricing_adjustment_non_binary;
    }
