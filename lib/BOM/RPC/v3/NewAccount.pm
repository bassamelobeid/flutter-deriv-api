...
sub verify_email {
    ...
    if (BOM::Platform::User->new({email => $params->{email}}) && $params->{type} eq 'lost_password') {
        ...
        send_email({
            ...
            message            => [BOM::Platform::Context::localize('<p>Dear Valued Customer,</p><p>In order to verify your withdrawal request, please click on the following link: </p><p> ' . $params->{link} . ' </p>')],
            use_email_template => 1
        });
    }
