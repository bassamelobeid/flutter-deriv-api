...
sub paymentagent_transfer {
        ...
        if ($client_to->get_status('cashier_locked') || $client_to->documents_expired) {
            return $reject_error_sub->(
                localize('There was an error processing the request.') . ' ' . localize('This client cashier section is locked.'));
        }
        ...
        if ($error) {
            ...
            if ($error =~ /BI102/) {
                ...
                return $error_sub->(
                    localize('An error occurred while processing request. If this error persists, please contact customer support'), $error
                );
            }
        }
        ...
        my $emailcontent =
            localize('Dear [_1] [_2] [_3],', $client_to->salutation, $client_to->first_name, $client_to->last_name,) . "\n\n" . localize(
