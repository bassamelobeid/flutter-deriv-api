...
rpc get_settings => sub {
    ...
    delete $settings->{fatca_declaration}    unless defined $settings->{fatca_declaration};
...
rpc set_settings => sub {
    ...
    if (scalar keys %$updated_fields_for_track) {
        ...
        BOM::Platform::Event::Emitter::emit('sync_user_to_CTRADER', {loginid => $current_client->loginid});
