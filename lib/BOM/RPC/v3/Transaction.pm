...
sub buy {
    ...
    my $client = $params->{client}
      // die "client should be authed when get here";
    ...
    my $purchase_date =
      time;   # Purchase is considered to have happened at the point of request.
    $contract_parameters =
      BOM::RPC::v3::Contract::prepare_ask($contract_parameters);
    ...
    try {
        die
          unless BOM::RPC::v3::Contract::pre_validate_start_expire_dates(
            $contract_parameters);
    }
    catch {
        warn __PACKAGE__
          . " buy pre_validate_start_expire_dates failed, parameters: "
    ...
    try {
        ...
        warn __PACKAGE__
          . " buy produce_contract failed, parameters: "
          . Dumper($contract_parameters);
        $response = BOM::RPC::v3::Utility::create_error(
            {
                code => 'ContractCreationFailure',
...
# for the same case. Hence, 'trade' is passed as parameter.
    my $msg2 = BOM::Platform::Context::localize(
        'Permission denied, requires [_1] scope.', 'trade' );
    ...
    return BOM::RPC::v3::Utility::create_error(
        {
            code              => 'TooManyTokens',
            message_to_client => localize('Up to 100 tokens are allowed.')
        }
    ) if @{ $params->{args}->{tokens} } > 100;
    ...
    if ($found_at_least_one) {
        ...
        my $purchase_date = time
          ;   # Purchase is considered to have happened at the point of request.
        $contract_parameters =
          BOM::RPC::v3::Contract::prepare_ask($contract_parameters);
        $contract_parameters->{landing_company} =
          $client->landing_company->short;
        my $amount_type = $contract_parameters->{amount_type};
        ...
        try {
            ...
            warn __PACKAGE__
              . " buy_contract_for_multiple_accounts produce_contract failed, parameters: "
              . Dumper($contract_parameters);
            $response = BOM::RPC::v3::Utility::create_error(
                {
                    code              => 'ContractCreationFailure',
...
sub sell {
    ...
    my @fmbs = @{
        $clientdb->getall_arrayref(
            'select * from bet_v1.get_open_contract_by_id(?)', [$id] )
    };
    ...
    return BOM::RPC::v3::Utility::create_error(
        {
            ...
            message_to_client => BOM::Platform::Context::localize(
                'Unknown contract sell proposal')
        }
    ) unless @fmbs;
        ...
        return BOM::RPC::v3::Utility::create_error(
            {
                code              => $err->get_type,
                message_to_client => $err->{-message_to_client},
                message           => "Contract-Sell Fail: "
                  . $err->get_type
                  . " $err->{-message_to_client}: $err->{-mesg}"
            }
        );
    }
