        ...
        return BOM::RPC::v3::Utility::create_error(
            {
                ...
                message_to_client => BOM::Platform::Context::localize(
"Streaming for this symbol is not available due to license restrictions."
                )
            }
        );
    }
...
sub _ticks {
    ...
    return [
        map { { time => $_->epoch, price => $ul->pipsized_value( $_->quote ) } }
          reverse @$ticks
    ];
}
...
# This ohlc_daily_list is the only one will get ohlc from feed.tick for a period
    if ( $end_time - $start_time <= $granularity ) {
        ...
        my $first_stop =
          $start_time + ( $granularity - $start_time % $granularity );
        my $last_stop =
          $first_stop +
          $granularity * int( ( $end_time - $first_stop ) / $granularity );
    ...
    my $ul = $args->{ul} || return BOM::RPC::v3::Utility::create_error(
        {
            ...
            message_to_client => BOM::Platform::Context::localize(
                "Please provide an underlying symbol.")
        }
    );
        ...
        return BOM::RPC::v3::Utility::create_error(
            {
                ...
                message_to_client => BOM::Platform::Context::localize(
                    "Start time [_1] must be before end time [_2]", $start,
                    $end
                )
            }
        );
...
# if no start but there is count and granularity, use count and granularity to calculate the start time to look back
    if ( not $start and $count and $granularity ) {
        my $expected_start =
          Date::Utility->new( $end - ( $count * $granularity ) );
        ...
        unless ( $ul->calendar->trades_on($expected_start) ) {
            ...
            do {
                ...
                and not $ul->calendar->trades_on($expected_start) );
        }
    ...
    if ( $args->{adjust_start_time} ) {
        unless ( $ul->calendar->is_open_at($end) ) {
            ...
            unless ( defined $shift_back ) {
                my $last_day =
                  $ul->calendar->trade_date_before( Date::Utility->new($end) );
                if ($last_day) {
