...
use constant SKIPPABLE_BARRIER_ERRORS => [
    ...
    'NoReturn',              'StakeLimits',       'PayoutLimits',                'InvalidBarrierRange'
];
...
sub handle_batch_contract {
    ...
    for my $contract ($batch_contract->_contracts->@*) {
        ...
        my $barrier_key =
            ...
            ? ($contract->{supplied_high_barrier} || $contract->high_barrier->as_absolute) . ':'
            . ($contract->{supplied_low_barrier} || $contract->low_barrier->as_absolute)
            : ($contract->{supplied_barrier} || $contract->barrier->as_absolute);
        ...
        $price->{error}{skip_error} = _skip_barrier_error($price->{error}->{message_to_client}->[0]) if $price->{error};
        ...
        if ($offerings_error) {
            my $price_offering_error = {
                ...
                error    => {
                    ...
                    skip_error => _skip_barrier_error($offerings_error->{error}{message_to_client})
                },
                barrier_key => $barrier_key
            };
            if ($contract->two_barriers) {
                $price_offering_error->{error}{details}{barrier}  = $contract->high_barrier->as_absolute;
                $price_offering_error->{error}{details}{barrier2} = $contract->low_barrier->as_absolute;
            } else {
                $price_offering_error->{error}{details}{barrier} = $contract->barrier->as_absolute;
            }
