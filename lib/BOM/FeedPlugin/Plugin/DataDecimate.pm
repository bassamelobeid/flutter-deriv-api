...
Its used to update DataDecimate for selected market.
...
use constant TICK_BY_TICK_TO_MASTER_REDIS => $ENV{TICK_BY_TICK_TO_MASTER_REDIS} || 0;
...
sub _build__tick_source {
    ...
    # This ENV[TICK_BY_TICK_TO_MASTER_REDIS] and check is temporary
    # and 'ELSE' will be remove after moving TICK_BY_TICK services to 'feed-collector11' server
    if (TICK_BY_TICK_TO_MASTER_REDIS) {
        $redis_write = BOM::Config::Redis::redis_feed_master_write();
        $redis_read  = BOM::Config::Redis::redis_feed_replica();
    } else {
        $redis_write = BOM::Config::Redis::redis_replicated_write();
        $redis_read  = BOM::Config::Redis::redis_replicated_read();
    }
