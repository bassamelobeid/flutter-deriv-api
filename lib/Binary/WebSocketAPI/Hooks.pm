...
my %rate_limit_map = (
    ...
    kyc_auth_status_real           => 'trading_platform',
    ...
    kyc_auth_status_virtual           => 'trading_platform',
...
sub output_validation {
    ...
    if ($api_response->{msg_type}) {
        ...
        if (($add_prop_error_count > 0) && ($add_prop_error_count == $error_codes_count)) {
            ...
            $log->error(
                "Additional Property Error [ " . " error: $error_details ], make sure backend are aware of this error!, schema may need adjusting");
            return;
        }
