        ...
        $c->call_rpc(
            {
                ...
                success => sub {
                    ...
                    $req_storage->{id} = _feed_channel_subscribe(
                        $c,     $req_storage->{symbol},
                        'tick', $req_storage->{args}
                    );
                },
                response => sub {
                    ...
                    undef $api_response
                      ...
                      ;    # Don't return anything if subscribed ok
                    return $api_response;
                }
            }
        );
...
sub ticks_history {
    ...
    my $args = $req_storage->{args};
    if ( $args->{granularity}
        and not grep { $_ == $args->{granularity} }
        qw(60 120 180 300 600 900 1800 3600 7200 14400 28800 86400) )
    {
        return $c->new_error( 'ticks_history', "InvalidGranularity",
            $c->l('Granularity is not valid') );
    }
    ...
    if ( $style eq 'ticks' ) {
        ...
        return $c->new_error( 'ticks_history', "InvalidStyle",
            $c->l( 'Style [_1] invalid', $style ) );
    }
        ...
        $c->call_rpc(
            {
                ...
                rpc_response_cb => sub {
                    ...
                    if ( exists $rpc_response->{error} ) {
                        ...
                        _feed_channel_unsubscribe( $c, $args->{ticks_history},
                            $publish, $args->{req_id} );
                        return $c->new_error(
                            'ticks_history',
                    ...
                    my $feed_channel_cache =
                      $c->stash('feed_channel_cache') || {};

                    # check for cached data
                    if ( exists $feed_channel_cache->{$channel}
                        and scalar( keys %{ $feed_channel_cache->{$channel} } )
                      )
                    {
...
# both history and candles have different structure, check rpc ticks_history sub
                        if ( $rpc_response->{type} eq 'history' ) {
                            ...
                            @times{ keys %$cache } =
                              map { $_->{quote} } values %$cache;

                            # merge with response data
                            @times{ @{ $rpc_response->{data}->{history}->{times}
                                  } } =
                              @{ $rpc_response->{data}->{history}->{prices} };
                            @{ $rpc_response->{data}->{history}->{times} } =
                              sort { $a <=> $b } keys %times;
                            @{ $rpc_response->{data}->{history}->{prices} } =
                              @times{ @{ $rpc_response->{data}->{history}
                                      ->{times} } };
                        }
          ...
          # if no window is in response then update the candles with cached data
                    ...
                    delete $feed_channel_type->{$channel}->{cache}
                      if exists $feed_channel_type->{$channel};
    ...
    if ( exists $args->{subscribe} and $args->{subscribe} eq '1' ) {
        if (
            not _feed_channel_subscribe(
                $c, $args->{ticks_history},
                $publish, $args, $callback, 1
            )
          )
        {
            return $c->new_error(
...
sub _feed_channel_subscribe {
    ...
    if ( !$shared_info->{symbols}->{$symbol} ) {
        ...
        warn(
"To many callbacks in queue ($symbol), possible redis connection issue"
            ...
            sub {
                ...
                while ( my $cb = shift(@$callbacks) ) {
                    ...
                    try {
                        ...
                        warn(
"callback invocation error during redis subscription to $symbol: $_"
                        );
                    };
                }
            }
...
# as we subscribe to transaction channel for proposal_open_contract so need to forget that also
    _transaction_channel( $c, 'unsubscribe', $args->{account_id}, $uuid )
...
sub process_transaction_updates {
    ...
    if ($channel) {
        ...
        foreach my $type ( keys %{$channel} ) {
            ...
            {
                _transaction_channel( $c, 'unsubscribe',
                    $channel->{$type}->{account_id}, $type );
            }
            else {
                $args =
                  ( exists $channel->{$type}->{args} )
                  ? $channel->{$type}->{args}
                  : {};

                my $id;
                $id =
                  ( $channel and exists $channel->{$type}->{uuid} )
                  ? $channel->{$type}->{uuid}
                  : undef;
                ...
                if ( $c->stash('account_id') ) {
                    if ( $type eq 'balance' ) {
                        ...
                        $details->{$type}->{balance} =
                          sprintf( '%.2f', $payload->{balance_after} );
                        ...
                        if ( exists $payload->{referrer_type}
                            and $payload->{referrer_type} eq
                            'financial_market_bet' )
                        {
                            $details->{$type}->{transaction_time} =
                              ( $payload->{action_type} eq 'sell' )
                              ? Date::Utility->new( $payload->{sell_time} )
                              ->epoch
                              : Date::Utility->new( $payload->{purchase_time} )
                              ->epoch;
                            ...
                            $c->call_rpc(
                                {
                                    ...
                                    rpc_response_cb => sub {
                                        my ( $c, $rpc_response, $req_storage )
                                          = @_;

                                        if ( exists $rpc_response->{error} ) {
                                            Binary::WebSocketAPI::v3::Wrapper::System::forget_one(
                                                $c, $id )
                                              if $id;
                                            return $c->new_error(
                                                'transaction',
                                            ...
                                            $details->{$type}->{contract_id} =
                                              $payload
                                              ->{financial_market_bet_id};
                                            $details->{$type}->{purchase_time}
                                              = Date::Utility->new(
                                                $payload->{purchase_time} )
                                              ->epoch
                                              if ( $payload->{action_type} eq
                                                'sell' );
                                            $details->{$type}->{longcode} =
                                              $rpc_response->{longcode};
                                            $details->{$type}->{symbol} =
                                              $rpc_response->{symbol};
                                            $details->{$type}->{display_name} =
                                              $rpc_response->{display_name};
                                            $details->{$type}->{date_expiry} =
                                              $rpc_response->{date_expiry};
                                            return $details;
                                        }
                                    },
                                }
                            ...
                            $details->{$type}->{longcode} =
                              $payload->{payment_remark};
                            $details->{$type}->{transaction_time} =
                              Date::Utility->new( $payload->{payment_time} )
                              ->epoch;
                            $c->send( { json => $details } );
                        }
                    }
                        ...
                        and $payload->{financial_market_bet_id} eq
...
my %skip_symbol_list =
  map { $_ => 1 } qw(R_100 R_50 R_25 R_75 R_10 RDBULL RDBEAR);
my %skip_type_list = map { $_ => 1 }
  qw(CALL PUT DIGITMATCH DIGITDIFF DIGITOVER DIGITUNDER DIGITODD DIGITEVEN);
...
sub _skip_streaming {
    ...
    my $atm_contract = (
        $args->{contract_type} =~ /^(CALL|PUT)$/
          and not( $args->{barrier}
            or ( $args->{proposal_array} and $args->{barriers} ) )
    ) ? 1 : 0;
    ...
    if ( defined $args->{duration_unit} ) {
        $skip_tick_expiry =
          (       $skip_symbols
              and $skip_type_list{ $args->{contract_type} }
              and $args->{duration_unit} eq 't' );
        $skip_intraday_atm_non_fixed_expiry =
          (       $skip_symbols
              and $skip_duration_list{ $args->{duration_unit} }
              and $atm_contract
              and not $fixed_expiry );
    }
...
sub _generate_uuid_string {
    local $/ = \16;
    return join "-", unpack "H8H4H4H4H12",
      ( scalar <$RAND> or die "Could not read from /dev/urandom : $!" );
}
