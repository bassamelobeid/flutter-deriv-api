...
subtest "get_financial_market_bet_buy_transaction_id" => sub {
    ...
    dies_ok {
        $txn_dm->get_financial_market_bet_buy_transaction_id();
    }
    'Dies when no financial market bet id is specified';
    ...
    subtest "When there are buy transactions for the financial market bet" => sub {
        my @buy_fmb_txn_id = grep { $_->{action_type} eq 'buy' } @{$transactions};
        ok(scalar @buy_fmb_txn_id, 'We have at least one buy transactions available');

        lives_ok {
            for my $txn (@buy_fmb_txn_id) {
                my $id       = $txn->{id};
                my $fmb_id   = $txn->{financial_market_bet_id};
                my $found_id = $txn_dm->get_financial_market_bet_buy_transaction_id({financial_market_bet_id => $fmb_id});
                is($found_id, $id, "Found txn id $found_id matches fmb buy txn id $id");
            }
        }
        'get_financial_market_bet_buy_transaction_id lives on all available buy fmb transactions';
    };

    subtest "When there is no transaction for the financial market bet" => sub {
        lives_ok {
            my $found_id = $txn_dm->get_financial_market_bet_buy_transaction_id({financial_market_bet_id => 10});
            is($found_id, undef, "Returns undef if no match found for fmb");
        }
        'get_financial_market_bet_buy_transaction_id lives if no matched financial market bet is found';
    };
};
