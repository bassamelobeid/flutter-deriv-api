...
lives_ok {
    ...
    $range_id = $range->financial_market_bet_record->id;
}
...
cmp_ok($range_bet_by_orm_object->financial_market_bet_record->payout_price, '==', 200, 'Check the parent paramt to see if they are loaded properly');
...
ok((
                $range->financial_market_bet_record->account_id eq $account->id
            and $range->financial_market_bet_record->underlying_symbol eq 'frxUSDJPY'
            and $range->financial_market_bet_record->payout_price == 200
            and $range->financial_market_bet_record->buy_price == 20
            and $range->financial_market_bet_record->sell_price == 40
            and $range->financial_market_bet_record->remark eq 'Test Remark'
            and $range->financial_market_bet_record->start_time->datetime() eq '2010-12-02T12:00:00'
            and $range->financial_market_bet_record->expiry_time->datetime() eq '2010-12-02T14:00:00'
            and $range->financial_market_bet_record->is_expired == 1
            and $range->financial_market_bet_record->is_sold == 1
            and $range->financial_market_bet_record->bet_class eq 'range_bet'
            and $range->financial_market_bet_record->bet_type eq 'RANGE'
            and $range->financial_market_bet_record->short_code eq 'RANGE_FRXUSDJPY_100_30_APR_09_1_MAY_09_13450_13000'
            and
