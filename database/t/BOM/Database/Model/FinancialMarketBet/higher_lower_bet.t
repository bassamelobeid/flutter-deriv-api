...
lives_ok {
    ...
    $higher_lower_id = $higher_lower->financial_market_bet_record->id;
}
...
cmp_ok($higher_lower_by_orm_object->financial_market_bet_record->payout_price,
...
subtest 'Expect that all fields are the same after loading FROM account transfer record' => sub {
    is $higher_lower->financial_market_bet_record->account_id, $account->id;
    is $higher_lower->financial_market_bet_record->underlying_symbol, 'frxUSDJPY';
    is $higher_lower->financial_market_bet_record->payout_price, 200;
    is $higher_lower->financial_market_bet_record->buy_price, 20;
    is $higher_lower->financial_market_bet_record->sell_price, 40;
    is $higher_lower->financial_market_bet_record->remark, 'Test Remark';
    is $higher_lower->financial_market_bet_record->start_time->datetime(), '2010-12-02T12:00:00';
    is $higher_lower->financial_market_bet_record->expiry_time->datetime(), '2010-12-02T14:00:00';
    is $higher_lower->financial_market_bet_record->is_expired, 1;
    is $higher_lower->financial_market_bet_record->is_sold, 1;
    is $higher_lower->financial_market_bet_record->bet_class, 'higher_lower_bet';
    is $higher_lower->financial_market_bet_record->bet_type, 'FLASHU';
...
lives_ok {
    my $new_higher_lower_bet = BOM::Database::Model::FinancialMarketBet::HigherLowerBet->new({
        ...
        'financial_market_bet_record' => $higher_lower->financial_market_bet_record,
    });
}
