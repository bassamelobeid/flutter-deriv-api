...
lives_ok {
    ...
    $touch_bet_financial_bet_id = $touch_bet->financial_market_bet_record->id;
}
...
cmp_ok($touch_bet_by_orm_object->financial_market_bet_record->payout_price,
...
is($touch_bet->financial_market_bet_record->account_id,        $account->id,       'account_id');
is($touch_bet->financial_market_bet_record->underlying_symbol, $underlying_symbol, 'underlying_symbol');
is($touch_bet->financial_market_bet_record->payout_price,      $payout_price,      'payout_price');
is($touch_bet->financial_market_bet_record->buy_price,         $buy_price,         'buy_price');
is($touch_bet->financial_market_bet_record->sell_price,        $sell_price,        'sell_price');
is($touch_bet->financial_market_bet_record->expiry_time,       $expiry_time,       'expiry_time');
is($touch_bet->financial_market_bet_record->is_expired,        $is_expired,        'is_expired');
is($touch_bet->financial_market_bet_record->is_sold,           1,                  'is_sold');
is($touch_bet->financial_market_bet_record->bet_class,         $bet_class,         'bet_class');
is($touch_bet->financial_market_bet_record->bet_type,          $bet_type,          'bet_type');
is($touch_bet->financial_market_bet_record->short_code,        $short_code,        'short_code');
