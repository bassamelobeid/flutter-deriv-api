...
sub _build_touch_bet_record {
    my $self = shift;
    if ($self->financial_market_bet_record->can('touch_bet') and $self->financial_market_bet_record->touch_bet) {
        return $self->financial_market_bet_record->touch_bet;
    } else {
...
#    my $args = shift;
#
#    $self->_delete_orm_object({'record' => $self->touch_bet_record});
