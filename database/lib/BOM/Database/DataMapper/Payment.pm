...
sub get_total_withdrawal {
    my $self     = shift;
    my $args_ref = shift;
    my ($start_time, $excludes, $to_currency) = @{$args_ref}{'start_time', 'exclude', 'to_currency'};
    my $dbh = $self->db->dbh;
    my @bind_values;
    ...
    try {
        ...
        if (my $payment_hashref = $sth->fetchrow_hashref()) {
            ...
            if ($to_currency) {
                $total_withdrawal = amount_from_to_currency($total_withdrawal, $payment_hashref->{currency_code}, $to_currency);
            }
        }
    }
    catch {
...
=item get_total_credit_card_deposit

Get total deposit amount of credit card deposit for account.

=cut

sub get_total_credit_card_deposit {
    my $self = shift;
    ...
    my $payment_record = BOM::Database::AutoGenerated::Rose::Payment::Manager->get_payment(
        ...
        query           => [
            't2.client_loginid'    => [$self->client_loginid],
            't2.currency_code'     => [$self->currency_code],
            't1.payment_type_code' => {eq => $BOM::Database::Model::Constants::PAYMENT_TYPE_CREDIT_DEBIT_CARD},
            't1.amount'            => {gt => 0},
        ],
        db => $self->db,
    );
...
=item get_first_funded_amount_of_account

get amount of first fund of account (currency dependent)

=cut

sub get_first_funded_amount_of_account {
    my $self = shift;
    ...
    if (scalar @{$payment_record} > 0 and $payment_record->[0]->amount) {
        return $payment_record->[0]->{'amount'};
    }
...
=item get_first_funded_currency_of_client

get currency of first fund in all currencies

=cut

sub get_first_funded_currency_of_client {
    my $self = shift;
    ...
    if (scalar @{$payment_record} > 0 and $payment_record->[0]->amount) {
        return $payment_record->[0]->account->{'currency_code'};
    }
...
sub is_duplicate_payment_by_remark {
    my $self   = shift;
    my $remark = shift;
...
Check the remark, date, amount of payment to find duplicate
