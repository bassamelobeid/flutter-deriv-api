...
my $broker_codes = $rt->broker_codes;
my @brokers      = $broker_codes->get_brokers_on_server($localhost);
my $broker_db    = BOM::Platform::Data::Persistence::ConnectionBuilder->new({
        operation   => 'write',
        broker_code => $brokers[0]->code,
    })->db;
...
if (request()->param('perform_actions')) {
    try {
        ...
        foreach my $todo (grep { /^fmb_/ } (keys %{request()->params})) {
            ...
            my $fmb =
              BOM::Platform::Data::Persistence::DataMapper::FinancialMarketBet->new({broker_code => $client->broker})->get_fmb_by_id([$fmb_id])->[0];
            ...
            if ($action eq 'cancel') {
                ...
                push @msg,
                  'We have closed the open position at 0 price and refunded the purchase price of ' . $bet_info->{currency} . $bet_info->{buy_price};
