        ...
        if ($new_doc_status eq 'delete') {
            ...
            foreach my $field ('issue_date', 'document_id', 'comments') {
                ...
                if (defined $input{$input_key}) {
                    ...
                    if ($field eq 'issue_date' && $to_update_value ne (eval { Date::Utility->new($to_update_value)->date_yyyymmdd; } // '')) {
                        $full_msg .=
                    ...
                    if (Date::Utility->new($to_update_value)->is_before(Date::Utility->new->minus_time_interval('1y'))) {
                        $full_msg .=
                            "<div class=\"notify notify--warning\"><b>ERROR: $file_name is too old, must have been issued within the last 12 months</div>";
                        $field_error = 1;
                        next;
                    }
...
if ($input{whattodo} eq 'uploadID') {
    ...
    foreach my $i (1 .. 4) {
        ...
        {
            print qq[<p class="notify notify--warning">Issue date "$issue_date" is too old, it must have been issued within the last 12 months.</p>];
            code_exit_BO(qq[<p><a class="link"href="$self_href">&laquo; Return to client details<a/></p>]);
        }
