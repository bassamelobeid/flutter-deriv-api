...
use warnings;
no warnings
  'uninitialized';   ## no critic (ProhibitNoWarnings) # TODO fix these warnings
...
my $self_href       = request()
  ->url_for( 'backoffice/f_clientloginid_edit.cgi', { loginID => $loginid } );
...
if ( $input{whattodo} eq 'sync_to_DF' ) {
    ...
    my $df_client =
      BOM::Platform::Client::DoughFlowClient->new( { 'loginid' => $loginid } );
    ...
    if ( not $currency ) {
        BOM::Backoffice::Request::template->process(
            ...
            {
                message =>
'ERROR: Client never deposited before, no sync to Doughflow is allowed !!',
                error    => 1,
                self_url => $self_href,
            },
    ...
    my $doughflow_loc =
      BOM::Platform::Config::third_party->{doughflow}->{ request()->brand };
    my $doughflow_pass =
      BOM::Platform::Config::third_party->{doughflow}->{passcode};
    ...
    if ( $result->{'_content'} ne 'OK' ) {
        BOM::Backoffice::Request::template->process(
            ...
            {
                message =>
"FAILED syncing client authentication status to Doughflow, ERROR: $result->{_content}",
                error    => 1,
                self_url => $self_href,
            },
        ) || die BOM::Backoffice::Request::template->error();
    ...
    BOM::Backoffice::Request::template->process(
        ...
        {
            message =>
              "Successfully syncing client authentication status to Doughflow",
            self_url => $self_href,
        },
    ) || die BOM::Backoffice::Request::template->error();
    code_exit_BO();
...
if ( $input{whattodo} eq 'uploadID' ) {
    ...
    foreach my $i ( 1 .. 4 ) {
        ...
        my $document_id =
          substr( encode_entities( $cgi->param( 'document_id_' . $i ) ), 0,
            30 );
        my $comments =
          substr( encode_entities( $cgi->param( 'comments_' . $i ) ), 0, 255 );

        if ( not $filetoupload ) {
            $result .=
"<br /><p style=\"color:red; font-weight:bold;\">Error: You did not browse for a file to upload.</p><br />"
              if ( $i == 1 );
            next;
        }

        if (   $doctype =~ /passport|proofid|driverslicense/
            && $document_id eq '' )
        {
            $result .=
"<br /><p style=\"color:red; font-weight:bold;\">Error: File $i: Missing document_id for $doctype</p><br />";
            next;
        }
        ...
        {
            $result .=
"<br /><p style=\"color:red; font-weight:bold;\">Error: File $i: Missing date for $doctype</p><br />";
            next;
        }
...
"<br /><p style=\"color:red; font-weight:bold;\">Error: File $i: Expiration date($expiration_date) error: $error</p><br />";
            ...
            elsif ($submitted_date->is_before($current_date)
                || $submitted_date->is_same_as($current_date) )
            {
                $result .=
"<br /><p style=\"color:red; font-weight:bold;\">Error: File $i: Expiration date should be greater than current date </p><br />";
        ...
        if ( $broker eq 'JP' ) {
            $client->citizen($docnationality)
              if $docnationality
              and ($docnationality =~ /^[a-z]{2}$/i
                or $docnationality eq 'Unknown' );
        }
        elsif ( $doctype =~ /passport|proofid/ )
        {   # citizenship may only be changed when uploading passport or proofid
            if ( $docnationality and $docnationality =~ /^[a-z]{2}$/i ) {
                $client->citizen($docnationality);
            }
            else {
                $result .=
"<br /><p style=\"color:red; font-weight:bold;\">Error: Please select correct nationality</p><br />";
                next;
            }
        }
        elsif ( !$client->citizen )
        { # client citizenship presents when uploading docs (for all broker codes)
            $result .=
...
'SELECT * FROM betonmarkets.start_document_upload(?, ?, ?, ?, ?, ?)',
        ...
        die 'start_document_upload in the db was not successful'
          if ( $error_occured or not $id );

        my $new_file_name = "$loginid.$doctype.$id.$docformat";

        my $checksum =
          BOM::Backoffice::Script::DocumentUpload::upload( $new_file_name,
            $file_contents, $file_checksum )
...
'SELECT * FROM betonmarkets.finish_document_upload(?, ?, ?)',
        ...
        die "Cannot record uploaded file $filetoupload in the db"
          if ( $error_occured or not $query_result );

        $result .=
"<br /><p style=\"color:#eeee00; font-weight:bold;\">Ok! File $i: $new_file_name is uploaded.</p><br />";
    }
    print $result;
    code_exit_BO(
...
if ( $input{edit_client_loginid} =~ /^\D+\d+$/ ) {
    ...
    unless ( $client->is_virtual ) {
        foreach (qw/last_name first_name salutation/) {
            if ( length( $input{$_} ) < 1 ) {
                code_exit_BO(
"<p style=\"color:red; font-weight:bold;\">ERROR ! $_ field appears incorrect or empty.</p></p>"
                );
            }
        }
        if (
            !grep( /^$input{'salutation'}$/,
                BOM::Backoffice::FormAccounts::GetSalutations() )
          )
        {    ## no critic (RequireBlockGrep)
            code_exit_BO(
"<p style=\"color:red; font-weight:bold;\">ERROR ! MRMS field is invalid.</p></p>"
            );
        }
    }
    ...
    if ( BOM::Backoffice::Auth0::has_authorisation( ['Marketing'] ) ) {
        ...
        if ( my $promo_code = uc $input{promo_code} ) {
            if (
                (
                    LandingCompany::Registry::get_currency_type(
                        $client->currency
                    ) // ''
                ) eq 'crypto'
              )
            {
                code_exit_BO(
'<p style="color:red; font-weight:bold;">ERROR: Promo code cannot be added to crypto currency accounts</p>'
                );
            }
            else {
                ...
                if (
                    !BOM::Database::AutoGenerated::Rose::PromoCode->new(
                        %pcargs)->load( speculative => 1 )
                  )
                {
                    code_exit_BO(
"<p style=\"color:red; font-weight:bold;\">ERROR: invalid promocode $encoded_promo_code</p>"
                    );
                }
    ...
    $client->payment_agent_withdrawal_expiration_date(
        $input{payment_agent_withdrawal_expiration_date} || undef );
    ...
    foreach my $existing_client ( map { $user->clients_for_landing_company($_) }
        qw/costarica maltainvest/ )
    {
        my $existing_client_loginid =
          encode_entities( $existing_client->loginid );

        if ( $input{professional_client} ) {
            $existing_client->set_status( 'professional', $clerk,
                'Mark as professional as requested' );
        ...
        if ( not $existing_client->save ) {
            $result .=
"<p>Failed to update professional status of client: $existing_client_loginid</p>";
        }
    }
    ...
    if ( my @matching_keys = grep { /tax_residence/ } keys %input ) {
        my $tax_residence =
          join( ",", sort grep { length } @input{@matching_keys} );
    ...
    foreach my $key (@number_updates) {
        if ( $input{$key} =~ /^(|[1-9](\d+)?)$/ ) {
            ...
            code_exit_BO(
qq{<p style="color:red">ERROR: Invalid $key, minimum value is 1 and it can be integer only</p>}
            );
        }
    }
  ...
  CLIENT_KEY:
    foreach my $key ( keys %input ) {
        if ( $key eq 'dob_day' ) {
            my $date_of_birth;
            $date_of_birth = sprintf "%04d-%02d-%02d", $input{'dob_year'},
              $input{'dob_month'}, $input{'dob_day'}
        ...
        if ( my ( $document_field, $id ) =
            $key =~ /^(expiration_date|comments|document_id)_([0-9]+)$/ )
        {
            my $val = encode_entities( $input{$key} // '' ) || next CLIENT_KEY;
            my ($doc) = grep { $_->id eq $id }
              $client->client_authentication_document;    # Rose
            next CLIENT_KEY unless $doc;
            my $new_value;
            if ( $document_field eq 'expiration_date' ) {
                try {
                    $new_value = Date::Utility->new($val)->date_yyyymmdd
                      if $val ne 'clear';
                }
                catch {
                    my $err = ( split "\n", $_ )[0]
                    ...
                    print
qq{<p style="color:red">ERROR: Could not parse $document_field for doc $id with $val: $err</p>};
                    next CLIENT_KEY;
                };
            }
            ...
            if ( not $set_success ) {
                print
qq{<p style="color:red">ERROR: Could not set $document_field for doc $id with $val: $_</p>};
                next CLIENT_KEY;
            }
...
# so we update this encrypted field only on value change - we don't want our trigger log trash
            ...
            my $secret_answer =
              BOM::Platform::Client::Utility::decrypt_secret_answer(
                $client->secret_answer );
            $secret_answer = Encode::decode( "UTF-8", $secret_answer )
              unless ( Encode::is_utf8($secret_answer) );

            $client->secret_answer(
                BOM::Platform::Client::Utility::encrypt_secret_answer(
                    $input{$key}
                )
        ...
        if ( $key eq 'age_verification' ) {
            if ( $input{$key} eq 'yes' ) {
                $client->set_status( 'age_verification', $clerk,
                    'No specific reason.' )
        ...
        if ( $key eq 'client_authentication' and $input{$key} ) {
            ...
            if ( $input{$key} eq 'NEEDS_ACTION' ) {
                $client->set_status( 'document_needs_action', $clerk,
                    'Documents uploaded' );
            }
            else {
...
"<p style=\"color:red; font-weight:bold;\">ERROR : Could not update client details for client $encoded_loginid: MIFIR_ID line too long</p></p>"
    ...
    if ( not $client->save ) {
        code_exit_BO(
"<p style=\"color:red; font-weight:bold;\">ERROR : Could not update client details for client $encoded_loginid</p></p>"
        );
    }

    print
      "<p style=\"color:#eeee00; font-weight:bold;\">Client details saved</p>";
    code_exit_BO(
        qq[<p><a href="$self_href">&laquo;Return to Client Details<a/></p>]);
}
...
if ($payment_agent) {
    ...
    foreach my $column ( $payment_agent->meta->columns ) {
        ...
        print "<tr><td>$column</td><td>=</td><td>"
...
my $client_info = sprintf "%s %s%s", $client->loginid, ( $name || '?' ),
  ( $statuses ? " [$statuses]" : '' );
...
my $new_log_href =
  request()->url_for( 'backoffice/show_audit_trail.cgi', $log_args );
print
qq{<p>Click for <a href="$new_log_href">history of changes</a> to $encoded_loginid</p>};
...
foreach my $l ( sort keys %$siblings ) {
    my $tokens =
      BOM::Database::Model::AccessToken->new->get_all_tokens_by_loginid($l);
    foreach my $t (@$tokens) {
        $t =~ /(.{4})$/;
...
BOM::Backoffice::Request::template->process(
    'backoffice/client_edit_upload_doc.html.tt',
    {
        ...
        countries => Brands->new( name => request()->brand )
