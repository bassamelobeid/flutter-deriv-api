...
subtest 'advertiser Registration' => sub {
    ...
    my $p2p_adv_client = P2P->new(client => $adv_client);
    $adv_client->account('USD');

    cmp_deeply(
        exception { $p2p_adv_client->p2p_advertiser_create() },
    ...
    ok my $adv = $p2p_adv_client->p2p_advertiser_create(%params), 'create advertiser';
    ...
    my $advertiser_info = $p2p_adv_client->p2p_advertiser_info;
...
subtest 'Duplicate advertiser Registration' => sub {
    ...
    cmp_deeply(
        exception {
            P2P->new(client => $client)->p2p_advertiser_create(name => $name)
        },
...
subtest 'is_visible flag and subscription event' => sub {
    ...
    BOM::Test::Helper::Client::top_up($client->client, $client->currency, 9);
...
subtest 'subscriptions' => sub {
    ...
    my $p2p_client1 = P2P->new(client => $client1);
    $client1->account('USD');

    cmp_deeply(
        exception { $p2p_client1->p2p_advert_info(subscribe => 1) },
        {error_code => 'AdvertiserNotRegistered'},
        'must be advertiser to subscribe all'
    );

    my $advertiser_id = $p2p_client1->p2p_advertiser_create(name => 'x')->{id};
    ...
    cmp_deeply($p2p_client1->p2p_advert_info(subscribe => 1), $expected_response, 'response when subscribe to all with no ads');
    ...
    cmp_deeply($p2p_client1->p2p_advert_info(subscribe => 1), $expected_response, 'response when subscribe to all with an ad');
    ...
    my $p2p_client2 = P2P->new(client => $client2);

    $client2->account('USD');

    my $resp = $p2p_client2->p2p_advert_info(
        id        => $advert->{id},
        subscribe => 1
    );
